...
#define SVM_CONFIG_FIELDS(X) \
    ...
    X(bool, zero_store_zr, "SVM_ZERO_STORE_ZR", DefaultOn, true, "A64 普通整数 StoreUniform/StoreMemory/SetHostFPR 的 LoadImm(0) value 直接选 wzr/xzr；所有 use 均为兼容 store 时抑制物化；缺省 ON，=0 回退") \
    ...
    X(bool, xmm_resident_hi, "SVM_XMM_RESIDENT_HI", DefaultOn, true, "XMM8-15 固定驻留 v24-v31；缺省 ON，=0 回退，依赖 SVM_XMM_RESIDENT；无害性依赖 SVM_FPR_SCRATCH_PRECISE+SVM_FPR_IPV_RECLAIM 同开；跨 unit ABI，原 translator/x86/translator.cpp") \
    ...
    X(bool, region_edges, "SVM_REGION_EDGES", DefaultOn, true, "region edge 内部化（bounded64）；缺省 ON，=0 回退；原 translator/x86/translator.cpp:730") \
...
// A-class P0-B lazy token ABI. Default OFF. Not a FeatureSet field: the
// last_result home is process-wide. =0 is today's eager x26 pack. =1 pins
// x12, skips hot-path PF/AF pack, and publishes at observe/exit points.
// Does not imply BACKEDGE_LATCH.
