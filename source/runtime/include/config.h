...
enum class Optimizations : std::uint32_t {
    ...
    DeadCodeRemove = 1 << 11,
    XmmFaultSink = 1 << 12,
    // Bit 13 stays reserved: global_opts is hashed into persisted JIT cache and
    // AOT validity keys, so reusing it would change the meaning of old keys.
    All = UINT32_MAX
};
...
struct Config {
    ...
    // Effective ARM64 code-shape policies. Keep these as Config inputs (in
    // addition to their SVM_* environment selectors) so AOT/JIT cache
