...
权重的 RE=0 重采从 SVM 3.335622 host/guest 降至 3.267832；以未变的 FEX
1.549 为分母，对应 2.153×→2.110×。旧表与这次重采的 unit 形成参数不完全相同，
因此不直接覆盖原表；按两种口径合看，当前正式 smallpt 距离 FEX 约 2.11–2.14×。
...
三项合计使正式 smallpt 默认 region host 减少 23,670,507（1.791%）。
...
RSB pop 复用末尾 dynamic `SetLocation` 可机械少一条 `LDR current_loc`，但正式 smallpt
上限仅 0.076%，c-ray 0.313%，不立项。剩余完整 `SetHostFPR` 中 `VecFUnary` 仅 0.163%，
其 lowering 不是单条完整写；不把 producer 白名单泛化到缺少原子性证明的 opcode。
...
- `SWIFT_FUZZ_SEED=123456` 全套件最终为 183 passed / 35 个既有 failed cases；
  1,047,656 passed / 44 failed assertions。与前臂相比没有新增失败位置。
...
1. 正式 smallpt 的 boundary 仍约 23%，其中 link 约 13%。下一步只接受具有独立
   fault/SMC/RSB 证明的 call/return 或公开出口子类；direct-link、region 扩窗与跨边
   state forwarding 的历史 NO-GO 结论不因总桶较大而自动重开。
