...
x86_64 guest → 自定义 IR → host ARM64 JIT(vixl) 的 DBT 主干在真实 glibc/musl 静态二进制上端到端验证通过；多线程 guest(clone/futex)、TSO 内存序、SMC 自修改代码（含 MT 安全回收）、SSE2 基线、x87(opt-in JIT）均已落地；FlagsElimination 放开 Carry Gate B（块内全路径覆盖死写删除）；翻译阶段七项分解探针（SVM_PROF2）、单块专用 RegAlloc 快路径（byte-identical）、UniformElim 早退剪枝、IR 构建消重（输出恒等）、decode 前端细分探针（SVM_DECODE_PROF）与 lowering 桶细分探针（SVM_LOW_PROF）、top-N distorm 快速通道、vixl EmissionCheckScope 快路径均已落地；syscall 面已覆盖七个主流 benchmark 语料（coremark/stream/smallpt/sqlite-speedtest1/c-ray/7zip/openssl-speed 全部跑通且输出与原生一致），FEX 对比基线已建立；执行侧优化落地：XMM0–15 静态驻留 + SSE NaN 快路径（W13 双开关默认 OFF，组合 geomean 1.205）、UniformElim 路径交汇（W14 默认 ON，coremark +8.6~8.9%）、x86 AES-NI/PCLMUL/SHA256 NI（W15，aes-128-gcm ~9×；SHA 随信号根治默认转正，sha256 7.5×）+ PCLMULQDQ imm=0x11 直发 PMULL2（W21）、ZExt32→64 单节点折叠（W19 默认 ON，32 位 GPR 写每处省一条 host mov）+ XMM uniform 转发开关化（W22 默认 ON ≡ 现状，审计量化既有收益）+ rt_sigreturn 内核语义重写（W20f，P0#3 根治：删 guest 栈私有帧，uc+fpstate ABI 恢复）+ 标量 SSE tied-destination/FEAT_AFP scalar-insert（W23 默认 ON，smallpt 1.38×）+ 标量 SSE 操作数 V 类化（W27 默认 ON，smallpt 再 1.23×）+ GHASH/AES FEX-style lowering（W28 默认 ON，GHASH 热 unit −31.9%）+ 窄 load 融合/GPR 常量 shift（W29 默认 ON，coremark +3.8%；项B 后经 baseline2 事故→根因修复→恢复默认，见 W34）+ XMM load→load SSA 转发（W31 默认 ON，GHASH 再 −7 条）+ 精确 NaN cold path（W37 默认 ON，smallpt 1.274×）+ unit-local flags 四件套（W38 默认 ON，coremark +4.86%）+ 结构化 AddressMode（W39 默认 ON，STREAM 几何 +3.6%）；x64 decode 入口重构（W40 纯重构，Decode 473→3 行、DecodeSwitch 六家族分文件，最大 McCabe 511→140）；SHUFPS 直降 NEON（W45）、SMC dirty hint（W46）、COMIS 紧凑 flags（W47）、JIT cache key 修 warm 全 miss（W48）均已落地（默认 OFF 开关，待翻盘）；GPR 全 pin 前置四件套 W50–W53 已合并（uniform 失效范围本地化、x87 TOP 专用寄存器退役、VIXL scratch 逐发射契约、Linux guest=host 恒等映射，全部默认 OFF）；pin 三阶段 W55/W56/W60 落地（16 GPR 全驻留能力交付，level 3 实测净负维持 opt-in）、XMM fault sink（W57）、SSE4.2 string 内联（W58）、branch-only flags（W59）、Linux 全量测试根修（W61）均已合并；**翻盘波次收官：SHUFPS_NEON/XMM_FAULT_SINK/FLAGS_BRANCH_ONLY/PIN_EXT=2+XPOOL 四项经受载 A/B 裁定翻为默认 ON（c-ray 1.44、smallpt 1.26、7z +5.0%、coremark 1.22 受载中位），六项维持 OFF；baseline4 受载全矩阵 SVM/FEX：coremark 0.403、sqlite 1.160、zip7 0.566、osslsha 2.494 胜、cray 因间歇停滞失真待 W64；W63 已根修 pin2/3 的 host fault 恢复缺陷（Linux 目标系统正确性门恢复全绿）；W64 已根修 c-ray 间歇停滞（退出中断竞态 → host 控制循环活锁，见下）；W65 落地 Linux x18 按 unit 条件化保留（spill 兜底 scratch，零 spill unit 逐位不变，全局保留方案实测 −3.6% 已否决）；Linux 全量 `all` 构建两处既有断裂已修（trampolines 死调用拖入 x86 前端未解析符号、jit_env 枚举与 SYS_* 宏冲突），orb 默认目标首次全量构建通过；W67 RA 形态只读探针 SVM_RA_SHAPE_PROF 落地（默认 OFF，OFF 指纹零 diff；七语料实测裁定 P1/P5 不立项、P3 静态推断确认、P2 收益面有限但真实，见下）；W69 c-ray 差距归因裁定（负载超敏证伪，真因=热块指令形态，见下）；W70 核查关闭 W44 候选 #4（CL shift 残余 83.2% 已被 W59/FlagsElimination 吸收，RA 安全原型静态上限 −0.1282% 且 A/B 负向，不落地）；W72 核查关闭 W44 末项 unit-local GPR cache（PIN2 默认后动态 GPR state 访问较 W44 降 83.03%，无边界静态上限 0.3987%、乐观上限 0.9877% 均 <2%，PIN3 净负机制复证为缩池 spill +3.514%——W44 队列清空）；W71 热点动态归因只读探针 SVM_RA_HOT_COALESCE 落地（默认 OFF，OFF 指纹零 diff；实测推翻 W69 的 macOS 平台形态外推——Linux 侧 0x402fed 零 spill，真热点 0x402e70 全程序上限仅 1.82%；c-ray 全程序机械占比 move/桥 31.04%、NaN 6.59%、state 合并 1.90%、spill 1.82%，见下）；W73 严格 leaf helper preserve_all 原型落地（默认 OFF，Clang 限定——GCC 下开关惰性，orb A/B 为同路径噪声不构成性能证据，见下）；W74 筛选关闭 move/tied-copy 方向（a+b 可删仅 3.21% < 5% 门，极宽上限 3.97%——c-ray move/桥 31% 中 ~90% 语义必需）；W75 核查关闭 proven-finite NaN 消除（生产可接受证明命中 0/143，乐观上界 1.62% < 2% 门；跨 unit 事实需改"任意 guest PC 可独立入场"契约或 guard+deopt，非低风险 spike）；W76 STREAM 不对称分解（Copy=memcpy 80B/轮摊薄固定成本 5×；FEX 同循环实码 10–12 条 vs 我们 36–47；W13 旗标对在当前 master 已反为净负，推荐撤销；下一步候选 = self-backedge flags local 只读设计审计，见下）；W78 Linux FlagM/FlagM2 getauxval 检测补齐（W77 根因 B 生产修复，cmp;adc 反相 carry 在 Linux 走 CFINV——指纹精确两 unit 48→45 IR，mac 逐位不变，见下）；W79 self-backedge 去物化审计关闭 W76 原始形态（自环裸 B(label) 不可被信号/SMC 抢占系 master 既有缺口，实证 alarm 自环 rc=124；fault 单条目无 pending-flags 配方），收窄批准 infra-first 两阶段默认 OFF spike（Scale 36→32/Add 39→35/Triad 47→43，已立 W81，见下）；W80 XMM_STATIC 修饰开关 SVM_XMM_POOL_EXT 落地（默认 OFF——NaN 冷 ABI v11–v14 租赁制还池 12→16，附带测量坐实 +4 headroom 不能翻转 W13 旗标对 STREAM 净负，见下）；W83 指纹 golden 平台/feature 分片落地（darwin/linux × flagm/noflagm 四 shard 已安装，orb pre-existing 18 行 st_blksize 漂移关闭，见下）；W82 X87 scratch 契约根修（W52 遗留的 fld_m80/StoreInt/Compare 预算漂移，helper-fault 38/38 恢复，见下）；W85 coremark post-pin 重归因（move/宽度桥 35.10% 成首账、spill 精确为零、W36 的 GPR state 账号已被 pin 波次吸收；新立 W86 整数宽度 canonicalization audit，见下）；W84 Clang preserve_all 真实 A/B（Clang 17 激活实证 DirectPreserveAll=4，7zip +0.744% 唯一显著但不足翻盘线，维持 OFF，W73 前置条件闭环，见下）；W81 self-backedge exit latch + flags 最小活跃集下沉两阶段 spike（双开关默认 OFF——P0 latch 实证修复 master 自环信号不可抢占缺口，P1 STREAM +6.4~13.9% 但 CoreMark −0.844% 硬门失败维持 opt-in，见下）；W86 整数宽度 canonicalization 筛选过门（CoreMark top-10 严格可删 6.34% ≥ 5%，7zip 9.06%；批准 SVM_RA_INTWIDTH_TIE 默认 OFF spike 立 W88，见下）；W87 FEAT_AFP NaN guard 消除审计过门（FPCR.AH=1+NEP=1+DN=0 对 x86 默认 MXCSR NaN 语义位精等效、orb Linux 矩阵逐位复证；STREAM 12.53%/smallpt 10.54%/c-ray 6.55% 动态可删，批准 SVM_SSE_AFP_NAN 默认 OFF spike，见下）；W88 整数宽度链 tied-copy 落地并直接翻默认 ON（7zip 中位 +7.34%/配对 CI[+6.38%,+8.23%] 决定性、coremark +0.35% CI 过零、smallpt/STREAM 噪声内 null、零回归；指纹双态逐位不变免 golden 重生成；=0 现场回退，见下）；W89 FEAT_AFP NaN guard 消除落地（SVM_SSE_AFP_NAN 默认 OFF——P0 FPCR 边界全闭环+P1 Add/Sub/Mul/Div/Sqrt 发码白名单；A/B：STREAM Add +10.70%/Triad +22.92% 决定性、Scale +2.14%、Copy null，smallpt −1.09%/c-ray −4.71% 真实小回退（边界税），裁定维持 opt-in 不翻默认，见下）；W90 FPCR 边界税全案收官并翻盘（一期边界缓存 + 二期诊断单次穿越实测 29.076ns 账目闭合 + 三期 HostFpEffect helper 透明标注产品化 c-ray 覆盖 99.623%；c-ray 回退 −5.03%→CI 跨零 null、smallpt 三组 CI 全跨零 null、STREAM Triad/Add +22.98%/+11.69% 保留——**SVM_SSE_AFP_NAN 翻默认 ON**，=0 现场回退，指纹双态逐位不变免 golden 重生成，见下）；direct link v2 全战役收官并转正（单 4B site 全量路径、旧 v1 机制与开关全删，coremark link leaf 动态账 42.2%→12.1%、smallpt 23.7%→8.9%，见下）；代码质量口径首例翻盘：MEM_HOSTBASE_FOLD + INDUCT_TIE 翻默认 ON（STREAM 动态 −13.96%，LINK_SUFFIX_COMMON 残值归零维持 OFF）；老跨块任务 #65/#66 重评 NO-GO 关闭（单块 unit 无作用域 / 候选为零，见下）；function-level 无收益根因调研收官（根因=边语义未内部化非单元尺寸，eager 共同热块实测净增 1.1–2.9%）；region 边内部化 P0/P1 已落地（`0311401`，默认 OFF——branch-only 动态净账实测约零、smallpt 0.0053%，价值在 dual-entry 基建支撑后续跨边状态转发，见下）；指纹脚本 FlagM 探测失败改 fail-loud（`ef6b9bb`，杜绝 sandbox 静默落错 shard 的假漂移警报）；**Linux 默认恒等映射落地（`3a02f32`——恢复原地址寻址设计意图：未设 env → identity，`SVM_MEM_IDENTITY=0` 显式回退 bounded bias 隔离，撞图 WARNING 后回退窗口续跑，macOS 不变；Linux 默认态 P-1 有界隔离转为 opt-in，见 §5 P-1）；move coalescing 逐条审计 NO-GO（普通可消池 0.805%/2.200%/≈0 全低于 5% 门，主体是 pin 家发布与真语义）；全 pin level 3 × identity orb 复测不翻盘（−28.60% vs level 2，轴终局关闭）；identity 野跳转根修（`8386d00`——W53 遗留：Linux identity 沿用 mac 16KiB 宿主页粒度，MapAnywhere 登记 4KiB 对齐地址而 Unmap 按 16KiB 扩界，误拆相邻 JIT RX 页致 L2/RSB 旧 host 指针跳野，busybox malloc/realloc/free=mmap→mremap→munmap 稳定触发；修为 Linux 4KiB 粒度 + Unmap 只释放 guest 所有权交集；附 `SVM_EXEC_TRACE` 默认 OFF 故障轨迹探针）；寻址修复点 A 落地并转正（`2820ec8` 落地默认 OFF → `4400aeb` 翻默认 ON，`SVM_ADDR_EA_TIE=0` 现场回退——A1 GetOperand 终端别名 RA tie 复用 W88 所有权转移消 mov，A2 identity 专用 composite EA 结构直进 memory IR、bias 逐字节不变；orb 实测动态 host 字节 bias A1 coremark −1.06%/smallpt −0.19%、identity A1+A2 coremark −2.83%/smallpt −0.79%、STREAM≈0，spill 全格 +0，12 格 oracle 全绿，darwin 指纹双态零差；linux 两片 golden 已随翻盘重生成，11 guest IR 只减不增合计 −2,412，=0 回退精确复现旧 golden）；寻址修复点 G 落地（`11d8ce7`，`SVM_ABS_CONST_MAT` 当时默认 OFF（现由 `b692fca` 翻为默认 ON）——绝对常量直发 result 删 scratch transport，smallpt 动态 −1.62%/−1.35%，残余 ~2.53% 所依赖的 ADRP/literal 两路线已实测否决）**；**GPR 全 pin 轴终局关闭 + scratch 预算模型诚实化翻盘（2026-08-05）**：混合 pin 级别 spike（SVM_PIN_DEMOTE，per-unit L3→L2 降级 + 自闭合 shim）机制全效（spill coremark 10.61%→2.34%、smallpt 8.98%→0.024%，达 L2 绝对值）但总账仍负（+5.06%/+1.60% vs L2）——实证 L3 抠掉 spill 后本体≈L2，第 13-16 条 pin 的毛收益在我们形态里约为零，level 2 已收割全部驻留红利，全 pin 轴前提层面关闭（不是塞不下，是没有值得塞的东西），diff 与报告归档 .cache/spike-archive/；同期归因实证 L3 内生撞线的 92.0%/89.9% 是 32/64 位 Add/Sub 被 opcode 级预算统一多收（窄宽 flags 对齐路径最坏峰值 5 vs 直发路径真实瞬态 1-2），窄宽路径只占 6.7%/5.6%——`SVM_SCRATCH_PRECISE` 实例化诚实计费落地（`ea37838`：按宽度+pseudo flags+右操作数形态合成预算，39 条价格断言 + 四语料 6,943 条发射记录 peak>budget=0 双重固化）并翻默认 ON（`b79c830`，=0 现场回退）——mac L2 coremark spill 3.04B→20（≈−100%，L2 全部 spill 均为预算谎言）、host_dynamic −1.44%，smallpt L2 spill −34.5%，L3 中幅正向（coremark spill −6.22%），Linux 中性（池基线含 x18 本已无 L2 spill）；orb 复验：linux-flagm/noflagm 指纹 ×OFF/ON 四组合逐字匹配 golden（IR 未变免重生成）、swift_test 178 例 1,040,347 断言、全 oracle、identity×precise 矩阵全绿。**spill 分配失败 farthest-end 驱逐落地（`a7eb761`，SVM_RA_SPILL_EVICT 默认 OFF）**：L3 剩余 spill 归因（99.9999%/97.49% 为 reserve/reload 正反馈）后的治本方案——分配失败时驱逐 end 严格最大的活跃区间（跳过 fixed-home/传入禁用寄存器，确定性决胜），整轮作废带 victim 重跑，Verify 不过则全部驱逐作废回退原 reserve ladder；mac 实测 L3 coremark spill −93.01%/host −10.17%（距 L2 仅 0.96%，交叉确认全 pin 收关前提）、smallpt spill −72.50%/host −7.29%、L2 中性、26 个 unit 真实回退 ladder。**同轮 orb 验证揪出已 push 默认配置 P0 并根修（`cb26d63`）**：L3+identity+precise+smallpt 翻译期静默 SIGABRT（rc=134 无断言文本，gdb 实锤 vixl UseScratchRegisterScope::AcquireNextAvailable ← LogicalMacro ← EmitPublishFCmpFlags）——非 compact 路径大掩码 And 非 logical immediate 需 VIXL 内部 scratch，原顺序在 GetTmpX() 之后，shared_tmp+packed 溢出重载+bit 占满 reserve=3 的池时 VIXL 无寄存器可借；此前被非 precise Add/Sub 的 5 预算抬高 unit reserve 掩盖，peak 审计只覆盖 bounded-bias Add/Sub（identity 形态+其他 opcode 是验证盲区）；修法零成本——大掩码 And 提前到 GetTmpX() 之前，峰值回落预算 3，修复后 identity×precise 四格矩阵+全 oracle+orb swift_test 全绿。**a7eb761 orb 八格密度验证收官（全 identity，修复后重跑）**：8/8 rc=0；Linux identity 下 L3 残存 spill 本就≈0（coremark 72 条、smallpt 0.017%——identity 直解 EA 消除了 bias EA 物化的触发源，与 mac bias 形态 25G/10% 差五到九个数量级），evict ON 方向全正（L3 smallpt spill 再 −74.4%、host −0.013%，L2 与 L3 core 中性），regalloc_ns 最差 +5.5% 于 0.25% 桶 = translate +0.013% 无长尾；**SVM_RA_SPILL_EVICT 已翻默认 ON（`9e70475`，=0 现场回退纯 ladder）**——mac bias 形态 −10.17% host 是大头，Linux identity 生产形态中性偏正，orb 默认态指纹 4400 unit 逐字匹配 golden（IR 未变免重生成）、swift_test 与全 oracle 两态复验绿。**SVM_* 环境开关集中化落地（`a7f644d`）**：138 个 SVM_*/SWIFT_* 开关收进 `svm_config.h` 一张 X-macro 字段表（类型/parser/默认值/迁移依据逐行声明），call_once 一次性解析成内存快照取代 ~40+ 处散落惰性 getenv，kGetenvNames 改由字段表生成（消灭注册漂移），ComputeEnvHash 改类型化序列化+未识别变量兜底，parse 语义与原站点逐位等价，驱动 ABI baseline 发布改直改快照；配套产出**进程级/可按 module 绑定分类表**（docs/svm-config-classification.md：A=79 进程级 / B=48 可绑 codegen / ?=11 缺证明待定），为按 backend::Module 绑定 FeatureSet（P2b 快照传递 + P3 绑定机制）铺路。门：mac/orb swift_test 179 例、darwin/linux 四 shard 指纹 4400 unit 逐字匹配、全 oracle。**B 类开关 FeatureSet 快照传递落地（`95919e0`，P2b）**：48 个 B 类开关从散点读全局单例改为翻译入口 resolve 一次、快照下传——decoder 经 decode 会话、passes 经 PassPipeline 显式参数、emitter 经 JitContext 构造期副本，per-unit 路径零反向全局读取（grep 门固化）；`ModuleConfig` 挂默认空 `FeatureOverrides`（sparse optional 数组），`ResolveFeatureSet` = 全局拷贝 + 逐条 apply，`ComputeConfigHash` 空覆盖逐位保持旧 cache identity、非空才进独立哈希域；两处穿线边界——XPOOL trampoline 改持 AddressSpace 最大并集掩码、RegAlloc 构造按 unit 快照收紧（默认可用池不变），`vixl_fast` 改 `JitScope(bool)` 发码会话显式值消除 VIXL 内部反向读全局；四项 Config 承载字段（sse_scalar_insert/sse_afp_nan/mem_hostbase_fold/induct_tie）本期不动，P3 收编建议已录（induct_tie/mem_hostbase_fold 可直接收编，另两项需组合 host FEAT_AFP capability）。行为零变化硬门：darwin/linux 四 shard 指纹 4400 unit 逐字匹配（含 host_bytes）、mac/orb swift_test 全绿、orb 全 oracle（coremark 0x4983、smallpt 8spp SHA 逐字）。P3（按 guest ELF 建 Module + override 填充 + jit_cache 多 Module 归属）已落地（见下）。**开关精简第一刀落地（`d6c18a1`，138→132）**：全 138 项逐条处置审计后用户裁决执行——D1a（翻盘开关删 =0 回退路径）一项不批（最老翻盘仅 6 天，失去 env 回退只剩代码回滚，口径：翻盘稳定数周后才谈删回退）；删除 D2 零读者 SVM_X87_TOPVIRT/SVM_PKRU（pkru 架构状态成员保留）+ D1b 默认-OFF 实验路径 SVM_LINK_SUFFIX_COMMON（direct-link v2 后残值精确归零，planner/suffix emitter ~210 行）、SVM_UNIFORM_PAIR_AUDIT（唯一战役 ≤0.16% NO-GO 收官，~210 行）、SVM_XMM_STATIC（W76 旗标对净负，静态驻留表/RA/xsave 同步 ~310 行；动态 residency、xmm_fault_sink、sse_nan_fast、XSAVE 布局保留）、SVM_XMM_POOL_EXT（v11-v14 扩展段，NaN 冷路径固定 ABI 本体保留；Optimizations bit 13 显式保留不复用，维持旧 cache validity key 位语义）；合计净删 1087 行，FeatureSet 48→46、SvmConfig 138→132，六枚 env 转未识别兜底哈希，darwin/linux 四 shard 指纹 4400 unit 逐字匹配、swift_test 全绿。**P3 按 backend::Module 绑定 FeatureSet 落地（`fd8a1d2`）**：linux driver 为主镜像 PT_LOAD 合并范围建独立 Module（`LoadedImage` 新增 main_start/main_end，interp/栈/堆仍归 default module，linker/ 未动）；绑定语法 `SVM_MODULE_FEATURES=main:field=0|1[,...]`（限 46 个 B 类 FeatureId，坏条目 WARNING 不 PANIC，不进 SvmConfig）；jit_cache 格式 v4→v5——SerialUnit 加 feature_hash（编译时所属 module 的 resolved FeatureSet FNV-1a 哈希），Load 逐 unit 按 guest_start 重查现属 module、hash 不一致 fail-closed 跳过（reject_feature 计数 + dirty），Load 时机从 AddressSpace::Init 移到 driver MapModule 后显式触发，Key() 聚合非空 override module 的排序 feature hash（空覆盖逐位保持旧 cache identity），MapModule 的 ownership LOG_ERROR 删除；示范 `main:ra_spill_evict=0`：smallpt 主镜像内单 unit 0x413cc8 host 1128B→2452B、spill_dynamic 1.9M→23.2M、主镜像外零变化。默认零行为变化硬门：darwin 双分片指纹 4400 unit 逐字匹配、swift_test 142 例全绿、空覆盖 Key() 逐位旧值。**sqlite VACUUM induction tie 误编译根修（`896af5a`)**：形态 `RAX+=32; RCX=RAX; RAX+=32; cmp [RCX+..]`(sqlite3RefillIndex 0x488b12)——旧 induction tie 只证明源在 Add 处死亡+结果首先发布回同一 pin，未证明 Add 结果 SSA 生命周期内固定家不被第二次 SetHostGPR 覆盖，RA 把第一次结果绑进 x22 后被第二次 RAX 发布覆盖，RCX 快照读出野 EA(sqlite speedtest1 测试 200 VACUUM 确定性 HaltReason::Signal,08-03 `989adbe` 翻 ON 引入；Carry Gate B 单独安全——=0 时保留的 SaveFlags observer 使旧 guard 拒绝该 tie，纯间接躲避非第二 bug;identity Unmap/恒等映射线已排除）；修法=RA 主证明扫描至结果最后使用点、遇结果仍活期间的同 pin 写拒绝 tie,emitter 按 block 实际 use(BitCast 穿透）独立复证同一硬门，证明失败仅回退原非 tie 分配不开新开关，SVM_INDUCT_TIE/SVM_FLAG_CARRY_ELIM 保持默认 ON；专项 RA 回归入 main_case(StoreUniform 保活第一次结果跨第二次同 pin 写，result 不得分 x22)。门：mac 套件 142 例+sqlite 五态（默认×3+两 =0 态）+darwin 双分片指纹逐字，orb 套件 176 例+sqlite×3+linux 双分片指纹逐字+全 oracle(coremark 0x4983、smallpt 8spp SHA 逐字）。master = `896af5a`。**baseline6 全矩阵（2026-08-06 安静窗口，REPS=5 warm，24/24 oracle，SVM/FEX 中位比）**：**sqlite 首次翻胜 0.911**（b5q 1.347）、**osslsha 2.458 胜保持**；smallpt 3.096→2.498、c-ray 3.395→2.640、coremark 0.428（受）→0.456、zip7 0.600（受）→0.662、osslaes 0.668（受）→0.683；STREAM Copy 0.707/Scale 0.332/Add 0.455/Triad 0.454。master = `f59b715`。**guest GPR 发布点 RA 合并器落地（`0c03c57`，SVM_RA_COALESCE 默认 OFF，FEX 差距方案 W-α）**：块内生命周期证明成立时把 guest GPR 完整宽度发布值直接分配到固定 pin 家省掉 SetHostGPR 回写、可复用 U32 GetHostGPR 读映射同 pin 快照，三类合并（同家回写/读复用/零扩展桥链），producer 白名单 LoadImm/LoadUniform/Zero/Add/Sub/And/Or/Xor、offset=0、U32/U64、pin 池 x0-x9/x22/x23/x29，partial write/跨块值/未证明高半部一律回退；证明链两层独立——RA 侧拒绝同 pin 交叉活跃值（复核轮补防：第三方 tie 值在 producer→store 窗口内定义、publish 在 store 之后的形态，与 sqlite induction tie 根修同一 bug 类），emitter 侧按实际 RA 寄存器与 last_use 独立复证、metadata 与复证不一致 ASSERT fail-loud；实测（mac 密度口径、EXEC_PROF 关闭）coremark host_dynamic −0.097%/smallpt −0.205%/c-ray −0.374%，净删 100% 落 move/宽度桥桶，spill/uniform 不动——安全子集机制成立但远未吃完 move 桶，默认翻 ON 需先扩白名单重过同一证明门；专项 24 断言（含交叉写窗口负测，测试专用 selector 预映射第三方值到 x22，不依赖具体 tie matcher 形态）。门：mac 套件 143 例 1,037,708 断言、darwin 双分片指纹 OFF 逐字匹配、ON 指纹 4400 unit 零变化、sqlite VACUUM ON 三轮全绿。master = `0c03c57`。**RA 合并器扩池并翻默认 ON（`778d67c` 扩池 + `18d7a65` 翻盘，=0 现场回退）**：producer 白名单逐 opcode alias 裁决后扩 20 个（Adc/Sbb/Mul/Div/AndNot/Not/六种 shift/rotate/ByteSwap/BitExtract/BitClear/Select/CondSelect/MulHigh U64),BitInsert(MOV;BFI 第一条 MOV 会毁被绑源）、可 fault 的 LoadMemory/atomic/helper(fault 边界可见状态）、partial write/BFI、跨块值维持否决并写明机制原因；每有效宽度实例四重门（OFF 无合并/ON 进 x22/第三方预绑冲突拒绝/ON 恰少一条 4B 发布）,297 专项断言；扩池后累计 coremark −0.115%/smallpt −0.340%/c-ray −0.380%（共同 PC 加权，净删 100% 在 move 桶，spill/uniform 不动，增量主战场 smallpt,c-ray 安全池一阶段已基本覆盖）。翻盘证据：oracle 两态逐字一致、指纹语料 4400 unit 两态零变化（darwin/linux 四分片 ON 态逐字匹配 golden，免重生成）、parser NonZero→DefaultOn 与其他翻盘开关同构；测试入口修正 RunForCoalesceConflictTest 首轮 RunVerified 显式 ra_coalesce=false（冲突绑定必须先于合并器唯一一次运行的时序，生产路径不受影响）。门：mac 套件 143 例、默认（ON)/=0(OFF）双态指纹逐字、sqlite 默认×3+=0×1 全绿；orb 778d67c 与翻后 18d7a65 两轮全绿（177 例、sqlite×3、linux 双分片逐字、全 oracle)。master = `18d7a65`。**unit 内常量地址寄存器缓存落地（`9ce4472`,SVM_CONST_ADDR_CACHE 默认 OFF，差距方案 W-ε）**：单 block 直线段内同一 U64 绝对地址 GetOperand（单 use 直喂 memory op）锚点正常物化、后续条目零发射复用同寄存器；缓存寄存器须全窗口 active mask 原本完全空闲（强于区间相交证明，第三方 SSA/fixed clobber/先前 tie 占用都排除），占用记入 active mask 后逐条重过 CheckInstr scratch 硬门、不足整体回退不驱逐活跃值，emitter 独立复证 fail-loud,anchor 纯常量计算不发 fault。实测纠偏：卡片 5-11% 系 top-unit 口径，精确同地址重复池远小于 movz/movk 形状粗估（smallpt 0x403ca0 四同页不同地址为反例），全程序 smallpt −0.067%/c-ray −0.020%/coremark≈0、spill/uniform 不动、oracle 两态逐字——机制成立安全边界已量化但收益不支持翻盘，维持 OFF,split interval/evict 激进扩池留待重评。专项 3 section（紧池回退、ra_coalesce 同开组合）10 断言。门：mac 套件 144 例、OFF 双指纹逐字、ON 指纹 4400 unit 零变化、sqlite ON×3;orb 全绿。master = `9ce4472`。**间接分支瘦身收官（`9d68ee1` 三开关 + `64dee80` shadow 翻默认 ON，差距方案 W-ζ）**：SVM_SHADOW_LEAN（=0 回退）——RSB frame 双 word 存稳定 L2 value-slot index,push 8→6/pop 14→12 条（先 post-index 消费 frame 再以 slot 直取 L2 key/value 对）,ON/OFF frame 混合 ABI 安全（互吃只损失命中率不野跳）,SMC 经既有 ClearDispatchSlots 先清 L2 value 同步失效；实测三语料一致为正 zip7 −0.238%/coremark −0.491%/smallpt −0.849%,spill/uniform 零变化，指纹语料双态零变化免 golden 重生成。SVM_INDIRECT_L1 维持 OFF——块内 L1 首槽快查 11 条（现场三条 hash+显式 L1 base,FEX 6 条靠 State 预存 pointer+mask),zip7 静态密度 +0.397% 反噬；配套无扰动探针 SVM_INDIRECT_L1_PROF(A 类永久 OFF,emitter 层固定 ip0/ip1 save/restore 计数、不进 IR/RA/scratch、扣除探针区间后发码逐位一致）实测 zip7 命中率 99.9998%(1.29 亿次 294 miss)，真实执行路径账 −0.159%——翻转前置=State ABI 预计算 L1 base/mask。专项 16 断言（开关独立性/形态回退/SMC 双表清零）。门：mac 套件 146 例、OFF 双指纹+四 ON 态指纹零变化、sqlite 四态各三轮；orb 两轮全绿。master = `64dee80`。**W-β flags 寄存器化裁定 NO-GO 带证据（`4137ba4` 并入 PF/AF density 六子桶只读探针）**：核心矛盾=跨块驻留即边界 ABI，与 B 类按 module 绑定 FeatureSet 结构性冲突（ON/OFF module 混跑需每边 pack/unpack shim 抵消核心目标）——凡跨块驻留类优化只能做 A 类进程 ABI（判例适用于后续同类项）；供给实测无免费 GPR(pool 扣 x6/x7 zip7 +1.09% 且 spill ×7；挪用 R10/R11 pin 的 x4/x5 coremark/zip7 ~+0.9%，未计 helper 边界税）；收益上限不足——PF/AF 动态 read 三语料全零无消费端红利、PF bfi→mov 仍 1 条（归零需 FEX 式 fixed-home RA affinity)、AF-only 理想上限 0.77-2.7% 盖不住供给税；重开条件存档（A 类裁定+AF-only 优先+affinity/snapshot/fault pack 不可分割 ABI spike)。W-δ 单元内化前提受创让位 W-γ。门：mac 套件 146 例、darwin 双分片逐字、sqlite 默认×3;orb 全绿。master = `4137ba4`。**W-γ XMM 组合调研收官（`07ebffc` 并入 XMM 密度只读探针）**：残余 XMM LoadUniform/StoreUniform 动态账 smallpt 14.59%/c-ray 14.33% host（占 uniform 账 85%/81%)，现有四项 unit-local 机制已吸收原始流量 62.5%/47.5%（收益按残余计）;FPR 类可进合并器机械上限 9.31%/8.41% host,GPR 视图 5.29%/5.92% 静态家只能 ldr/str→umov/ins 不归零；供给面分两期——XMM0-7 覆盖残余 65%/69% 且动态 FPR pool 保 20，全 16 家 pool 28→12 是 W76 罚单形态；开关按 W-β 判例只能 A 类进程级（拟名 SVM_XMM_RESIDENT)；实现前置阻塞清单固化（W-α 仅 GPR、GetHostFPR 绑家无生命周期扫描系 sqlite 同类洞、EmitSetHostFPR 空实现无回退、inline helper 未并入 FPR static descriptor)。P0(XMM0-7 组合：固定家+FPR 双闸合并器+emitter 回退+全边界）已立项实施。门：mac 套件 146 例、darwin 双分片逐字；orb 全绿。master = `07ebffc`。**W-γ P0 XMM0-7 固定驻留落地（`a2af332`,A 类 SVM_XMM_RESIDENT NonZero 默认 OFF,SvmConfig 137→138、FeatureSet 50 不变）**：guest XMM0-7 经 UniformMapDesc 绑 host v16-v23（动态 FPR 池 28→20 保 20)；防 W76 形态复现的关键排序——UniformElimination 首遍对驻留区访问 defer,StoreSink 之后 FinalizeStaticFPRMappings 才统一下发 GetHostFPR/SetHostFPR(XmmFaultSink pass 槽在 xmm_resident 下强制启用）;RA 新增 FPR 发布点合并器（完整 V128 last-use 写归零+read 快照复用，12-opcode producer 白名单 LoadUniform/LoadMemory/VecAnd/Or/Xor/VecAdd/Sub/Mul/VecFAdd/Sub/Mul/Div，三重守卫=双侧区间第三方冲突+窗口内同家 Get/Set 观察者+producer 输入活跃覆盖）,emitter 双闸独立复证 fail-loud;GetHostFPR/SetHostFPR 补齐真实回退（V128 orr、partial lane ins、GPR view umov/ins，修掉原空实现）;三个 live-interval collector 对 GetHostFPR 绑家前过 host_fpr_writes 生命周期扫描（sqlite induction-tie 同类洞）;XSAVE/XRSTOR helper 前后 spill/restore 驻留家、preserve_all 快照强制并入静态 FPR 家；fault 经既有 sigreturn/static save 路径，新增 v16-v23 互异 128-bit pattern 触发 guest page fault 逐 lane 校验专项。实测（禁 EXEC_PROF,host_static×entries 动态加权，净含 helper 税）:smallpt host_dynamic −2.196%(uniform −9.52%、move +7.31% 桥接代价、spill 精确不变、池 28→20)、c-ray −2.197%、coremark ≈0；合并方独立复测 smallpt ON host_dynamic=236,338,392,712 与报告逐位一致。门：mac 套件 OFF/ON 各 148 例、helper-fault ON 38/38、darwin 四分片（OFF/ON×flagm/noflagm）指纹全部逐字匹配 golden(0/4400，翻盘若发生免重生成）、sqlite 默认×3+ON×3、smallpt SHA oracle;orb 两轮——基础门（182 例、sqlite×3、linux 双分片逐字、全 oracle)+ ON 态加验（helper-fault 38/38、sqlite ON×3、linux 双分片 ON 逐字、全 oracle ON、XSAVE 冒烟）。w66 工位曾遭外部删除事件（1605 文件含 .git),patch 经逐文件核验 13 文件 +900/−31 与报告一致后方 apply。master = `a2af332`。**SVM_XMM_RESIDENT 翻默认 ON（`7112f44`,=0 现场回退）**:NonZero→DefaultOn 与既有翻盘开关同构；翻盘门——mac 套件默认（ON)/=0 双态 148 例、darwin 四分片默认/=0 指纹全部逐字匹配 golden(0/4400 免重生成）、sqlite 默认×3、helper-fault 默认 38/38;orb 翻盘态全绿（182 例、sqlite×3、linux 双分片逐字、全 oracle)。master = `7112f44`。**W-γ P1 部分家落地（`041a516`,A 类 SVM_XMM_RESIDENT_HI NonZero 默认 OFF,SvmConfig 138→139)**：完整高 8 家（池 20→12）经纯缩池探针双重证伪——smallpt 新增 1.89B/c-ray 542M 热 spill dynamic 真税，且 v31 是 VIXL MacroAssembler 默认 fptmp(d31)、装机即隐式 scratch 绕 RA；退阶落地 XMM8-11→v24-v27（池保 16),descriptor 表 16 项按 resident_count 取前缀、HI 严格依赖 base(base OFF 逐位惰性）,RA 范围谓词泛化 v16-v31、可用子集仍由保留掩码二次判定，P0 证明链无放松；v24-v31 供给面审计全边界（VIXL d31/NaN v11-v14/helper/XSAVE/fault/SMC/region slow link）天然覆盖。实测净账：smallpt −0.744%(spill 精确不变）、c-ray −0.247%(143k 新增 spill 税已扣）、coremark ≈0;**zip7 动态 ≈0 但静态 host bytes +3.02%——无 SSE 热池语料白付 descriptor 税，裁定维持 opt-in 不翻默认**；完整高 8 重开前置归档（VIXL d31 契约重做+池 12 spill 降回池 16 水位+净账重测，三项缺一不装机）。合并方独立复测 smallpt HI host_dynamic=234,580,380,425 与报告逐位一致。门：mac 套件 OFF/HI 148 例、helper-fault HI 38/38、darwin 四分片 OFF/HI 指纹逐字（0/4400)、sqlite 6 轮、smallpt SHA;orb 基础门+HI 态加验（helper-fault/sqlite/双分片/oracle）全绿。工位已迁 ~/CLionProjects/SwiftVM-w66(/private/tmp 删除事件后）。master = `041a516`。**inline L1 预计算落地并翻默认 ON（`b62548f` 落地 + `8a92edf` 翻盘，=0 回退，W-ζ 全案收官）**:State 复用冻结槽（reserved_linkage_address 原位改名 indirect_l1_code_cache，布局零移动，旧 State::l1_code_cache 仅非 latch 配置设置、latch/profile 形态需两次 pointer chase，新槽 Runtime 构造期无条件发布）;Runtime L1 改 direct low-bit hash(一条 and 兼 mask),L2 保持 folded;快查 11→7 条（ldr/and/add/ldp/cmp/csel/br),key mismatch csel x30 等价旧 ret;SMC 失效安全——TranslateTable 新增 invalid_value,SMC Zero 把 L1 value 原子改写为 dispatcher 新增 label_indirect_l1_miss 安全 L2 入口，旧 pointer 读者由既有 epoch/QSBR 保活，br 0 不可能；PROF 诊断形态保持 invalid=0 口径逐数一致。实测：mac zip7 命中率 99.999773%、orb Linux 99.999686%（盈亏线 46.15%)，真实执行路径 −7h+6m=**−0.2785% zip7**，与 shadow_lean 组合 entry-weighted 静态账打平（−0.000478%，旧实现 +0.158% 反噬消除）,spill/uniform 零变化；合并方复测命中率逐数一致、ON 密度 325,812,603,412 ≈ 报告值（162/3258 亿 entries 噪声）。门：mac 套件四态、四指纹逐字（0/4400 免 golden)、sqlite 6 轮、helper-fault 双态 38/38、SMC 专项；orb 落地态+翻盘态全绿+zip7 PROF 复测。FeatureSet 成员默认同步 true,B 类按 module 绑定语义保持。master = `8a92edf`。**baseline7 揪出两起翻盘回退（2026-08-07 安静窗口）**:① **openssl speed SIGALRM 饿死（L1 翻盘撤回，`09cadb6` 回 OFF)**——EVP 函数指针派发紧循环的间接出口被 inline L1 命中臂直吞,dispatcher 信号 safepoint 永不执行,guest SIGALRM 帧不得运行,首个 8s 块实测 882s 至 INT32_MAX(harness rc=124);=0 逐位恢复。重开前置=快查纳入 pending-signal 检查并重过两臂账,验收门补 openssl alarm 语料(本波所有 gate 缺的正是这个语料——suite/指纹/sqlite/helper-fault 全绿也照样漏)。② **STREAM 四核分化（xmm_resident 所致,复验中）**:SVM 侧 Scale 28.5k→19.0k/Add 43.1k→28.3k/Triad 43.0k→28.4k(−34%)、Copy 62.0k→86.8k(+40%);`SVM_XMM_RESIDENT=0` 逐位恢复 b6 水位，其余开关 =0 无效；HI 不救。sqlite 比率异动（0.911→1.271）系 FEX 侧 13.1s→9.7s 漂移,SVM 侧 12.3s≈b6 持平——FEX warm cache 跨基线不稳定,裁定只看 SVM 侧绝对值。smallpt 2.498→2.097/c-ray 2.640→2.213/zip7 0.662→0.678/coremark 0.456→0.453/STREAM Copy 首胜 1.042。**STREAM 回退根修落地（`6d0a3eb`)**:codex 归因到指令序列级——Scale/Add/Triad 热块每迭代双发布 XMM0(movapd 后 fault 快照 + mulpd/addpd 后终值),FinalizeStaticFPRMappings 把两处改写进同一家 v16,形成循环携带的同名寄存器短链,Apple ARM64 微架构罚 −31% 墙钟（尽管动态指令 −4.15%);排除实验逐一否掉搬家 v10/合并发布 mov 等替代,唯移出 XMM0 有效。修复=驻留区间 [0,8)→[1,8)(HI [1,12)),XMM0 留 canonical State,XMM1-7→v17-v23，池回 21。合并方复验五轮中位：Copy +42.3%(90,076 vs 63,310)、Scale/Add/Triad 回 OFF 水位（±0.3% 内，host 序列与 OFF 逐位一致）;smallpt 密度 −1.832%(spill 逐位不变）,AES-GCM 大块 +29%。门：mac 套件双态 148、四指纹逐字 0/4400、sqlite 双态×3、helper-fault 双态 38/38、openssl alarm 双态四格全 rc=0;orb 六段全绿。**翻盘纪律升级**:翻盘证据从此要求全七基准矩阵，三语料密度不够；openssl alarm 语料常驻门。master = `6d0a3eb`。**inline L1 信号 safepoint 补齐并重新翻盘(`b1a1310` 机制 + `30ea085` 翻 ON,=0 回退,FeatureSet 同步 true)**：方案=State 首字恒为 exit_request(latch OFF 也恒初始化，旧 L1 指针别名全仓零读者已核),SignalInterrupt 先写 halt_reason 再 release 置 bit63,LDAR 匹配 acquire;生产快查臂前置 LDAR+TBNZ(复用 index 暂存，零新增 scratch),热路 7→9 条，命中走冷 ret 复用 trampoline 完整保存路径。有界性：纯 L1 命中循环最坏延迟=一个额外命中边+下一次轮询，EVP 实测每 2s 块 1.98-2.01s 递达。真实路径账 −5h+8m,zip7 −0.1989%(命中率 99.9998% ≫ 新盈亏线 61.54%),zip7 墙钟五轮中位 −0.884%;静态供给税 zip7 +0.358%/host bytes +0.71%(184 site×36B)已明示。PROF 臂逐位未动。合并方复验：mac 套件双态、四指纹逐字 0/4400(语料无 eligible site,免 golden)、helper-fault 双态 38/38、sqlite 双态×3、openssl alarm 双态四格 12.2s 全 rc=0、STREAM 双态三核噪声内、zip7 ON 密度 326,254,624,948 ≈ 报告值(358/3268 亿 entries 噪声,spill 逐位同);orb 全绿+回退态加验。master = `30ea085`。**W-η 回边 safepoint 免税裁决 NO-GO（纯报告，零生产改动）**:codex 以有界性不变量先行+最小 guest 实证——zero-tax 需要 SwiftVM 没有的异步精确暂停/任意 JIT PC deopt map/跨核 branch patch 可见性基础设施（region_link_trampoline 既有注释：它核 cache maintenance 不保证目标核观察 patch，目标核不 slow traversal 就永不 ISB，循环依赖）;FEX 形态（tgkill 定向 pause+SRA spill）是独立基础设施项目，重开最小基建七步已记录（全 direct-edge patchpoint/transient pause generation/目标线程 kick/本核指令同步/relink 抑制/QSBR ack 闭环/先修一般 direct SCC)。机械税账：coremark 2.214%/zip7 0.430%/其余≈0，收益池真实但不足以抵正确性缺口；只删 self poll 非法（那是唯一实证有界的子集）。**裁决中揪出独立现状洞**:两块跨 unit direct cycle 在 SVM_BACKEDGE_LATCH=1 下仍无 observer——合并方用 codex 的最小 guest 在 master 二进制复测：self loop 1.04s 有界递达，two-block rc=124 超时，洞确认。修法=收费制 SCC 回边轮询补全 latch 覆盖（与零税优化分账），已派单。**direct SCC 覆盖根修落地（`363ebf0`)**：确定性下降边 cover——latch=1 下同 module 可绕 dispatcher 的 direct 边中 target<source_block_start 者一律收 LDAR+CBNZ（任意 ≥2 节点有向环沿 PC 序必有严格下降边，故不漏；保守多标合法；判定只依赖 guest PC/module/BlockLink，与编译/cache/SMC 顺序无关）;poll 在 MergeNZCV+FlushSpillWrites 后已提交边界，cold stub 发布边目标按 bit63 分 Signal/CodeMiss;direct-link/L2/region 三路统一前置 poll,LinkManager 摘链不触 poll。latch=0 逐位不变（两片指纹逐字 0/4400)。latch=1 税：coremark +2.76%/zip7 +1.93%/smallpt +0.64%/c-ray +0.68%(418-2089 sites×2 条，逐 PC 复算闭合）——opt-in 正确性补票， latch 维持默认 OFF，翻默认需另案减税。合并方复验：未打补丁二进制先行确认 latch=1 指纹 loop 6→8 差异系既有 latch 行为；套件双态 151 例、helper-fault 双态 38/38、sqlite 双态×3、openssl alarm 双态四格 12.2s、三 alarm guest(self/two-block/三块混合）latch=1 全 rc=0 ~1.02-1.45s 递达、zip7 latch=1 密度 333,988,671,892 ≈ 报告值；orb 全绿。master = `363ebf0`。**W-δ 重评收官（`14b11c0`，机制合并默认 OFF)**:W-β NO-GO 后拆分裁决——eager 128 块形态四语料实测 +1.04%~+13.10% 定量否决；**bounded 16 块窗 GO**:region_edges 态默认预算 1024/cap128 改 16(=2..127 可覆盖，=0 仍 eager),含完整 SCC 税净账 coremark −6.488%/smallpt −1.847%/c-ray −2.240%/zip7 −2.647% host dynamic,spill 零增；收益来源=function 级 FlagsElimination/Uniform/XMM finalize 等既有 pass 看到更大更贴热链的 unit，非跨边 flags/SSA 驻留（W-β 矛盾未借壳复活）。条件边单 b.cond 是其严格子形态。coremark 热边集中 core_bench_list 链表环（top1 边 887M entries)。**顺手修补 region-only 信号洞**:direct_cycle_latch 扩为 latch||region_edges(region-only 跨 region 闭环原可无 observer)，三 alarm guest × 三开关组合九格全 rc=0 ~1.02s。翻盘暂缓：翻译 +32~52%、code bytes +50~77% 供给税需启动/RSS/长运行 break-even 另案。合并方复验：OFF 双分片指纹逐字 0/4400(OFF 逐位不变硬证）、ON flagm 自洽 1781 units 且全零 block fallback、套件双态 151 例、helper-fault 双态 38/38、sqlite 双态×3、openssl alarm 双态四格 12.2s、join 口径复算 coremark ON 207,076,391,837 ≈ 报告值；orb 全绿。方法论资产：unit-aware direct graph 探针（默认 OFF)+ 基线 entries × candidate shapes 重加权 join(region 内部入口不过 entry counter，直接相加曾虚报 −60%)。master = `14b11c0`。**W-ε 扩池重评全项 NO-GO(`e195411` 收只读探针）**：现价池分解证明现行机制已抓尽同块精确重复池（smallpt 缓存覆盖 99.999998%/c-ray 99.999921%,**四语料 no_free=0**)——split interval 早已实现（缩窗回退即候选级 split),evict 没有可交换对象（仅剩各 1 条冷 verify 失败，机械上限 3 条动态指令）,direct SCC 内同地址跨块池=0（循环不变量无池）；跨块毛上限 smallpt 0.520%/c-ray 0.305% 但 region 多入口独立 L2 身份破坏寄存器继承前提，安全实现=跨块 SSA/dual-entry ABI（越界另案）；跨 unit 毛上限 1.59%/0.55% 需跨 unit ABI（同越界）。开关维持 OFF。探针（CONST_ADDR_CACHE=1 且 RA_HOT_COALESCE_ALL=1 双开才输出，组内闭合 ASSERT）已并，合并方复验：套件双态 151 例、指纹双分片逐字、audit 双开 smallpt 全程 748 组闭合、orb 全绿。master = `e195411`。**SVM_REGION_EDGES 翻默认 ON（`61ffc73` + linux 分片 `86d3f06`，首个非指纹中性翻盘）**：bounded16 region 转正。密度账（join 重加权）coremark −6.488%/smallpt −1.847%/c-ray −2.240%/zip7 −2.647% host dynamic、spill 零增；墙钟 coremark 200k −12.4%（合并方复核 −11.9%，15.60s→13.74s）、zip7 Tot +2.670%、sqlite −1.410%、STREAM −2.751%（Triad +22%）、c-ray long −0.851%；供给税=真实冷启动翻译增量 21.6~156.5ms（c-ray short +5.586%，无探针两点拟合 break-even 7.56s），host bytes +51~79% 但 RSS 峰值不变、热 cache 加载持平/下降；JIT cache key 已含 region 形态（ComputeConfigHash），双态各成 svmjit-*.bin 无跨形态复活（合并方复核 ON/OFF 两 key、warm 加载正常）。默认形态 4400→1781 function units（零 block fallback），darwin/linux × flagm/noflagm 四 golden 分片全部由合并方重生；SVM_REGION_EDGES=0 回退形态与旧 golden 逐字一致（4400 units，mac+orb 双侧验证，回退完好——此后 =0 不再匹配新 golden，属设计内）。门禁：swift_test 151 双态、helper-fault 38/38 双态、九格 alarm rc=0 + 负对照 rc=124、sqlite 双态×3、openssl sha256/aes-128-gcm 双态六段表、STREAM 双态 Validate（Copy 保持 87.8k）；orb 六段全绿（套件 185 例、sqlite×3、双分片、hello/func_tests/coremark 0x4983/smallpt SHA 全中）。master = `86d3f06`。**baseline7 全矩阵收官（2026-08-07,REPS=5 warm,24/24 oracle,results/baseline7/ + smallpt 补跑格）**：锚 baseline6(08-06）之后三波落地（STREAM 修复 6d0a3eb/L1 重翻 30ea085/region 翻盘 61ffc73）全部计入实测：Copy 62.1k→89.4k MB/s(+44.1%,SVM/FEX 0.707→0.997 统计平手）、osslaes +36.3%(0.683→0.913)、Triad +28.2%(0.454→0.563)、coremark +16.2%(0.456→0.509)、smallpt 墙钟 −14.0%(2.498→2.203)、c-ray −13.9%(2.640→2.322)、zip7 +6.4%(0.662→0.674)、osslsha 胜场 2.458→2.561;Scale/Add 持平（0.338/0.454)。sqlite 比率 0.911→1.238 纯系 FEX 侧 13.80s→9.99s 漂移（既有前科同形态）,SVM 绝对值 12.566→12.360s(−1.6%，与 region −1.41% 吻合）且 0.973 胜 Rosetta——裁定口径不变：只看 SVM 绝对值。方法论:harness SKIP_smallpt_svm 缺省=1，全矩阵必须显式 =0，否则 smallpt/svm 格静默 skip(oracle 行留 0 占位）；含 STREAM 回归污染的 prelim 残骸改名 baseline7-prelim-void 留存备查。master = `6f40c81`。**SVM_FLAGS_FCMP_COMPACT 翻默认 ON（`bb69403`，指纹中性翻盘）**：COMIS/UCOMIS compare 链 23 条→5+4 条，净省 14 条/compare 按 compare 次数严格闭合；密度（join 重加权）smallpt −14.446%/c-ray −7.690%,coremark/zip7 逐条中性，c-ray spill −50%;交错墙钟 smallpt −3.926%/c-ray −7.116%（五对全胜），合并方复核 smallpt −5.4%/c-ray −6.7%、翻盘后默认态 smallpt 10.31s(OFF 11.27s)。仅 FlagM2/AXFlag 主机启用（decoder HWCAP 硬 guard，无 FlagM2 逐字走默认路径）;COMIS 专项差分 3482 assertions 双态、signal-frame/host-call flags 最小 guest、helper-fault 38/38 双态、alarm 18 格、sqlite×3、openssl 六段表、矩阵 oracle 逐字全绿；当前指纹 corpus 无 COMIS 触发形态，四 shard 双态逐字零重生；cache env hash + per-unit FeatureSet hash 双隔离。整批矩阵曾现 zip7 −17.8% 窗口漂移假象，交错次序复核证伪——**整批顺序差值不作开关因果，判定一律看交错+动态账**。orb 六段全绿。master = `bb69403`。**SVM_SSE_AFP_MINMAX 机制合并（`5db1655`，默认 OFF）**：tied scalar VecFMinMax(RA last-use tie + emitter 同 FPR 复证）+ 有效 AFP 契约 → 单条 FMIN/FMAX;FPCR.AH 给 unordered/equal 第二源选择（与 x86 逐位一致，含 NaN payload/±0/SNaN 原样）,NEP 保 upper lanes；任一证明失败逐字原路径。账：c-ray host −1.294%（毛上限 1.636% 兑现 79%,tied 命中率 80.2%)、spill 零变；smallpt 仅 −0.098%(tied 25.7%)不翻默认；交错墙钟 c-ray −0.78%~−1.03%（合并方复核 −1.29%）。测试：发码形态 32 + JIT/解释器逐位差分 266 assertions 双态；套件 153 双态、指纹 OFF/ON 逐字、helper-fault 38/38 双态、alarm 18 格、c-ray BMP 逐字；orb 六段全绿。合并方踩坑复录：zsh `for gate in $order` 不分词致 gate="0 1" 整批无效（既录陷阱再犯）；c-ray 不认 -o，固定写 rendered_0000.png。master = `5db1655`。**SVM_ABS_CONST_MAT 翻盘复核 NO-GO（零 diff 收官）**：密度复现且略强（smallpt −2.103%/c-ray −0.480%,host delta 精确等于 move 桶减量，spill/uniform 逐条不变），但交错墙钟不兑现（smallpt +0.094%、c-ray paired −0.036%，均在噪声带）——2.1% 删 move 不在关键路径；历史 OFF 理由（2026-08-04 修复点 G：仅机制成立、翻盘证据不足）现价未推翻，维持默认 OFF 作 opt-in 代码质量开关。**发现潜伏跨开关测试耦合（合并方已在 master 二进制复现）**:main_case.cpp:5849 把 CONST_ADDR_CACHE 节省硬编码 3 条，ABS_CONST=1 时未缓存基线只剩 2 条 → =1 套件红 32==28（产品正确性门全绿，纯测试隔离缺口；最小修法=测试显式 override abs_const_mat=false)。cache 双 hash 门覆盖该开关；本机 launcher stored=0 不能伪装 revive 证据（纪律：cache 结论只引代码双门+专项）。master = `33c95ba` 不变。**SVM_SSE_SCALAR_TIE 翻默认 ON（`e5ff77b`）**：resident XMM 固定家 read 此前吃不到 scalar binary tie（GetHostFPR 跳过普通 interval 分配）,producer 落普通 FPR 双 copy;现复用 P0 FPR 发布合并器扩证八个 scalar 二元 producer(f32/f64 add/sub/mul/div):left 固定家 read 同家 + last-use 精确为 producer、producer 完整 V128 且末用精确为 SetHostFPR、第三方相交区间/observer/输入跨占一律拒绝，失败逐字原路径；依赖 scalar_insert AFP/NEP 契约，严格单 block。每命中精确省 2 条（读快照+发布同归零）:smallpt host −0.182%、c-ray −0.386%（新增 tie×2 严格闭合，spill 零变）;兑现毛池 6.42%/36.17%，剩余池需普通 FPR coalescing（不放松证明）。emitter 双闸（alu ASSERT + mem 复证）。前置修 main_case 5849 跨开关耦合（ABS_CONST=1 套件转绿）。测试：形态 90 + 差分 290 assertions 双态；套件 155 三态、指纹三格逐字（零 shard）、helper-fault 38/38 双态、alarm 9 格、sqlite/openssl/smallpt SHA 全绿；orb 六段绿。master = `e5ff77b`。 **SHUFPS 高频 imm tie 默认 OFF 合并(2026-08-08,master `a376d81`)**:smallpt/c-ray 归因候选 #5 收官——新 B 类开关 `SVM_SSE_SHUFPS_IMM`(双处登记,NonZero 缺省 OFF),RA 侧把 E4 及 alias E0/E5 挂入既有 ScalarInsertTieSource 所有权转移链,emitter 三处 ReproveShufpsImmTie fail-loud 独立复证(imm 白名单、非 fixed-home、共享 FPR、block 内精确末次使用);E4 alias 0 条、E0/E5 alias 单条 lane INS、E4 two-source 单条 qword INS 取代通用双表。c-ray 密度 -0.0467%(216,228,438 条,探针逐 tie 闭合,仅兑现毛上限 7.38%),spill 精确不变;主热点 56.64% 的 E0/E5 直接读 resident fixed-home,绑回原地毁 guest XMM 固定家,结构性拒绝,不属本 spike 可扩范围。smallpt/coremark/zip7 零动态命中。合并方复验:套件双态 156 cases、全 256 imm JIT/解释器差分 771 断言双态、OFF 双分片指纹逐字 golden(ON 亦逐字,corpus 无命中,指纹中性)、helper-fault 38/0 双态、alarm 18/18+负对照 124、sqlite/openssl/STREAM 双态、矩阵 oracle 双态逐字;交错三对 c-ray 墙钟纯噪声,证实「墙钟不兑现」结论,持 OFF。orb 六段全绿(linux 双分片指纹逐字、coremark 0x4983、smallpt 8spp SHA 15416623)。候选清单仅剩 AdvancePC NZCV merge unit-local DCE 审计(已派,可吃池 <0.5%% host 直接否决)。master = `a376d81`。 **AdvancePC NZCV merge DCE 审计 NO-GO(2026-08-08,master `35952ca`)**:smallpt/c-ray 归因候选清单收官项——审计先行裁决:严格 unit-local 可吃池=0(两语料),乐观上界仅 0.000618%%/0.000557%% host(低于 0.5%% 裁决线约 809/898 倍)。关键 emitter 复证:EmitVecFCmp 覆盖 host NZCV 前无条件 MergeNZCV+FlushFlags,审计中途的 0.5359%% 跨块伪池全部由它剔除——删 AdvancePC 只把同样四条 merge 搬到下一块 VecFCmp,净零。真正删除需跨块 flags 驻留/双版本状态协议,属 W-β 已否决区。交付仅双门惰性审计探针(translator.cpp +46/-2,CFG 身份+nzcv 脏位日志,默认零发码),不新增开关、无 ON 态。拒绝项主账:flags consumer 32.3%%/38.0%% 毛池、fault/helper/commit 边界 56.2%%/50.4%%、external terminal 11.5%%/11.6%%。合并方复验:套件绿、双分片指纹逐字、orb 六段全绿。至此归因报告五候选+本审计全部收官:FCMP_COMPACT 翻 ON(bb69403)、SCALAR_TIE 翻 ON(e5ff77b)、AFP MIN/MAX 并 OFF(5db1655)、ABS_CONST 复核 NO-GO(78834b1)、SHUFPS_IMM 并 OFF(a376d81)、AdvancePC DCE NO-GO(本锚);smallpt/c-ray 对 FEX 从 2.498/2.640 收至约 2.0-2.1/2.15-2.2,剩余差距主体为 move/桥 ~36%%(FEX 16-XMM fixed SRA 结构差,W-β 否决区)与跨边 flags ABI。master = `35952ca`。 **vixl V-scratch 契约排雷已并(2026-08-08,master `e74acb5`)**:用户批准的单——vixl 默认 fptmp_list_={d31},macro 暗租 scratch V 会静默覆写,XMM resident 若扩至 v31 即毁 guest 寄存器;全仓实证暗租站点仅两处(Fcmp 非零字面量:1483 与 PrintfNoPreserve:2563),生产路径零调用。补丁把既有 SvmBeginScratchContract 扩为 GPR/V 双掩码:契约激活期 V allow-list 恒空,AcquireNextAvailable 漏斗内 VIXL_CHECK 在取得寄存器后、发码前终止;GPR 侧 XPOOL 关闭时改按 stock tmp_list_ 校验(acquired 恒为其子集,不误火);jit_context 三个发码窗口默认全武装,EndVixlScratch 后端 ASSERT_MSG 复核 vreg==0。专项 fork death-test:契约下 Fcmp(d0,1.0) 子进程 SIGABRT;全套件(含 x87)武装态绿,生产 V scratch 需求实测为 0。纯排雷,不含 resident 翻盘——池 12/13 spill 经济学(smallpt +1.89B/c-ray +542M 热 spill)仍是 全映射独立否决项。合并方复验:套件 157 cases、双分片指纹逐字、helper-fault 38/0、alarm 9/9、sqlite×3、oracle 逐字;orb 六段全绿。master = `e74acb5`。 **XMM 静态映射供给曲线闭合 NO-GO(2026-08-08,master `83e2ff6`)**:用户指令推进 XMM 全静态映射,v31 排雷后的经济学总账——只读探针 `SVM_XMM_FPR_POOL_CAP`(钳池 13..20,三重 ASSERT,默认惰性)并 master,八池点×九语料臂供给曲线全量。结论:无甜点位。c-ray 池 15 进 spill cliff(+5.77B)、池 13 +43.0B spill/+10.80%% host;sqlite 零新 spill 但有非 spill 阶梯动态税(池 17 +0.246%%)——隐藏裁决者;静态税随家数单调增长。需求侧:七语料等权最优家序 13,10,12,11,15,8,14,9 而非顺排 8-11;STREAM 93%% XMM 流量在永久除名的 XMM0。逐家数净账(100%% 兑现乐观上限,不计桥/静态税)8/11/12/13/14/15 家全档在 sqlite/coremark/zip7 至少一项为负→全档 NO-GO,驻留维持 XMM1-7 现价,HI 续 OFF。合并方独立复现 c-ray 全曲线九个点,host/spill 与报告逐位一致(池13 +10.7959%%/+43,030,235,426)。重开前置:ray/crypto 专用 opt-in(进程启动前全局选择)+频率序子集 {13,10,12,11}+15 实测+sqlite/zip7/STREAM 无害门重过。门禁:套件 157 cases、指纹默认及 cap=0 双分片逐字、helper-fault 38/0、orb 六段全绿。master = `83e2ff6`。 **FPR 需求侧双机制已并(2026-08-08,master `ab45c03`,默认 OFF)**:供给曲线 NO-GO 的根因从供给改判需求虚高后的定向削减——①`SVM_FPR_SCRATCH_PRECISE`:scalar FP/VecFCvtPacked 按位宽/RHS/kind 实例定价取代 opcode 最坏值(VecFMulAdd 试降 4 被 GetTmpV 预算 ASSERT 当场拦截维持 5,fail-loud 安全网实证);②`SVM_FPR_IPV_RECLAIM`:有效 AFP NaN 契约下 cold ABI 不可达(QueueVecNaNColdPath ASSERT 绊线),trampoline 发布 AFP 并集、per-unit RegAlloc 按 FeatureSet 恢复/回收 v11-v14,自然池 21→25。实测:c-ray 池 15 cliff +1.439%% host/+5.77B spill 压到 +0.144%%/+32.8M(合并方主仓复测逐位一致);HI+A+B 组合 11 家驻留池仍 21,smallpt −0.949%%/c-ray −0.624%% host、c-ray 墙钟 −2.82%%(我复核 −2.85%%),spill 精确零增,sqlite/AES/STREAM/coremark 动态中性;zip7 墙钟 +0.292%% 未过严格全正门→双机制 OFF 合并,HI 候选资格仅在 A+B 同开成立,翻盘待 zip7 长复测裁决。门禁:套件双态 158 cases、专项 10 断言、OFF 双分片指纹逐字(ON 0 unit 变化)、helper-fault 38/0 双态、alarm 18/18、sqlite/openssl/STREAM 双态、oracle 逐字、orb 六段全绿。master = `ab45c03`。 **HI+A+B 三开关翻盘落地(2026-08-08,master `bfd8caa`,默认 ON)**:zip7 严格全正门最后一块补齐——合并方生产形态七对交错复测中位 −1.06%%(OFF 臂 25.68-28.10s ±5%% 窗漂,跨会话 12 对符号不一致,codex +0.292%% 税证伪),`SVM_XMM_RESIDENT_HI`/`SVM_FPR_SCRATCH_PRECISE`/`SVM_FPR_IPV_RECLAIM` 三开关同步翻 DefaultOn(FeatureSet 57-58 与 env 184-185 双位同步),=0 单开关回退;env 注释写明耦合:单独关 IPV_RECLAIM 时 HI 退化为池 17 有税形态。落地形态:XMM1-7→v17-v23 + XMM8-11→v24-v27 共 11 家驻留,自然池仍 21(4 回收抵 4 pin);c-ray 墙钟 −2.8%%(翻盘后默认态复测 26.67s vs OFF 基线 27.3s)、smallpt host −0.949%%、AES −0.70%%,sqlite 供给税消零,spill 精确零增。demand-lean 单测改显式构造双态(off.fpr_scratch_precise=false / keep.fpr_ipv_reclaim=false),不随默认值漂移。门禁:套件 158 cases 1039590 断言全绿、指纹默认态与全 =0 回滚态 darwin 双分片逐字 golden(golden-free 翻盘)、smallpt/c-ray oracle 逐字、orb 六段全绿(linux 双分片逐字、sqlite×3、coremark 0x4983、smallpt 8spp SHA 逐字)。master = `bfd8caa`。 **STREAM 定因 + fault 快照正确性修复翻盘(2026-08-08,master `1b6315d`)**:STREAM 定因(合并方静态复验全过):Copy 0.981 已带宽 parity;Scale/Add 耗时相同(16B/24B 分母假象);税序 flags 19.0%% > EA add 10.3%% > State 重载 8.3%% > XMM0 快照 6.2%%,Scale/Add flags 税根因=退出边 call gettimeofday 前无覆写 → branch-only 拒(Triad 有 `add $8,%r12` 故收);FEX 对照为 orb perf map 真提取。连带抓获真实正确性 bug:mulpd 完成后 addpd(mem) fault 时快照发布乘前值(最小 guest 实测 {1.5,-4} vs 期望 {3,-12},合并方双态复跑确认)——UniformElimination 反向 DSE 不把 faulting memory op 当观察点。修复两轮:round1 屏障形态(clear_xmm 复活中间 store,`78ff3f4`)被合并方四语料密度否决(smallpt +3.238%%/c-ray +2.574%% RA move 税,codex 只量 STREAM 漏报);round2 零税值选取(`e4de171`→翻盘 `24f369e`):DSE 前捕获边界最新完整 V128 SSA 瞬态计划,sink 在既有发布点改 source(Triad str q0→str q1 条数不变),resident FPR slot 免计划,搁浅例外全语料零触发。合并方 master build 四语料复测逐位级吻合:STREAM −1.3428%%/smallpt −0.1320%%/c-ray −0.0428%%/coremark +0.0000004%%,spill 全零增。golden 随 ON 态四分片再生(变更集=darwin/linux 一致:vec_float_nan_pressure 0x400078 1335→1267 + real_hello/real_busy/func_tests 同族三点,=0 回退不再匹配新 golden 属设计内)。门禁:套件双态 160 cases、专项 50+6 断言双态、helper-fault 38/0 双态、alarm 18/18、OFF 双分片逐字、oracle 双态逐字、orb 六段全绿(新 linux golden 逐字)。master = `1b6315d`。遗留:候选①loop 不变量 GPR 晋升(8.3%% 毛)②常量 hoist ③unit-local lazy flags(18.6%% 毛高风险)⑤XMM0 循环局部双缓冲排队。循环不变量晋升战役收官(合并 `67336e5` 缺省 OFF + 翻盘 `60c67b4` 双 ON):unit-local 单块自环形态,只读 guest GPR 基址(每环限 1 锚,≥2 长活基址在 Apple 短环触发无 spill 的寄存器供给反噬,Triad 实测 53.5→28-30GB/s)与 loop-end 宽常量(≥2 非零半字、唯一 Add/Sub 比较链)晋升至普通入口前缀,自回边直落循环体入口;全 unit StoreUniform 重叠扫描否写、固定映射排除、RA 双跑 spill 不劣才采用否则 Revert 还原原序。合并方 master build 复测:STREAM host 动态 −9.6844%%(Scale 27→24、Add 31→28、Triad 21→18 条/迭代),静态字节零差、spill 零增;七语料矩阵逐位吻合交付账(coremark/smallpt 精确零、c-ray −0.000066%%、sqlite −0.000263%%、zip7 −0.009074%%、openssl 噪声)。墙钟双会话交错 12 对:Triad +11.6~11.9%% 决定性翻胜(0.571→约 0.64 vs FEX),Copy/Add 中性,Scale 6/6 正负对半 |中位|<0.3%% 判带宽瓶颈噪声(−10.7%% 动态被访存 bound 遮蔽)。golden-free:ON/OFF 四组合 darwin 双分片逐字吻合既有 golden(纯序重排不改 unit 账),门禁双态全绿(套件、helper-fault 38/0、alarm 18/18、oracle 逐字)。回退:SVM_LOOP_GPR_HOIST=0 + SVM_LOOP_CONST_HOIST=0。遗留:候选③unit-local lazy flags(18.6%% 毛、fault/signal PC flags 物化证明风险最高)⑤XMM0 循环局部双缓冲(条件 GO)排队。lazy flags 研究开关收官(合并 `64d95fb` 缺省 OFF,翻盘 NO-GO):未新造 ABI,新 B 类 FeatureSet `flags_loop_lazy` 仅让 unit 选择既有已审计 BackedgeFlagsPlan(OR 关系,双 OFF 逐字同旧逻辑);严格子集=单块自环+唯一 cold 直接后继,cold exit/signal stub/fault veneer 先物化再暴露 context;JIT cache fail-safe 双闸。密度:Scale/Add flags 链 12→6 条/迭代,机械 join −17.95%%(合并方 master build 逐位复现 −6,194,999,573),实际稳态 −5.310B;七语料其余 ≤0.012%%,spill/uniform 零增。墙钟双会话 12 对交错否决:Scale −32.4%% 确定性回退(7/7 全负)、Add +45.5%%(7/7 全正)、零改动对照语料 Copy +5.6%%(7/7 一致)——Apple 核心 code placement 敏感实锤;重开前置=独立 loop-body alignment 研究 spike(Add +45%% 示天花板)。门禁双态全绿:套件、darwin 双分片 ON/OFF 逐字、helper-fault 38/0、alarm 18/18 + RE0L0 负对照 rc=124、oracle 逐字。遗留:候选⑤XMM0 循环局部双缓冲(条件 GO)排队;alignment spike 待立项。alignment 研究 spike 收官(NO-GO,零代码,基线 `32795b5`):五轴受控扫描——body 16/32/64/128B 对齐、unit 分配 64B-4KiB 对齐、回边距离 1-12 NOP、cold veneer 距离 0-256B、branch polarity 反转——全救不回 Scale(lazy=1 恒 19.1-19.7 vs OFF 28.8 GB/s,绝对 64B 对齐 19.43 与未对齐 19.47 等价);padding 静态税小但零收益,不开关。前提修正:Copy 0x416980 在 lazy=1 下 eligible(合并方 master build 复验),此前"Copy 零改动对照位移=placement 实锤"论断作废,真零资格对照是 Triad 0x401ba0。SVM_FLAGS_LOOP_LAZY 维持缺省 OFF;残留方向=逐指令替换(strb polarity/MRS/NZCV merge 组合)+ Apple PMU。临时审计探针已全部移除,tracked diff 为空,双态门禁复跑全绿。coremark 定因收官(纯研究零代码,基线 `c3e2282`,coremark-gap-report.md):SVM host/guest 4.079,move/宽度桥 38.54%%、boundary site 22.62%%(供给账非可删池)、flags 6.04%%、region poll 5.17%%、spill≈0。12 个热形态与 FEX orb 运行时真提取(perf-map+/proc/mem+objdump+post-RA IR 映射)逐条配对,精确闭合 78.50B=SVM host 28.43%%;墙钟锚 0.5027 与 0.509 口径吻合。合并方 master build 复现:top6 热块 entries×host/entry 逐项逐位一致(0x402df8 518.4M×47、0x402814 558.2M×27、0x402d58 518.4M×29、0x402680 1183.2M×11、0x402808 555.6M×19、0x402668 579.0M×15),总账 276,101,562,074。候选表:①SVM_RA_WIDTH_CHAIN 多节点宽度链合并 GO(毛 38.54%%,保守 6-9%% host,matrix 双环实证 8.26%%);②SVM_FLAGS_REGION_BRANCH 窄比较 branch-only 条件 GO(3.5-5.5%%,fault/signal 物化=最高风险);③删 signal poll NO-GO(W-η 边界)④boundary 当可删池 NO-GO(W-β 边界)⑤EA 小池 <1%% 不立项。无新正确性 bug;FEX 400k 控制 13.903s validated 证明抽码配置正确。宽度链合并收官(merge OFF NO-FLIP,锚点 `bd3ba4a`,width-chain-report.md):codex 交付 +900/−15 十文件;合并方 master build 全门独立复验——diff 十文件精读(RA 侧组件构造+双侧区间/fixed clobber/同家 SetHostGPR 扫描+DirtyGPR 回放 CheckInstr 失败恢复;emitter 独立复证 ASSERT fail-loud;新路径全 feature-gated),套件双态 PASS,darwin OFF 双分片逐字(ON flagm 亦逐字,指纹 corpus 零 eligible),helper-fault 双态 38/0,alarm 18/18,oracle 双态全绿(coremark 0x4983、smallpt/c-ray SHA 逐字、STREAM Validates、sqlite/zip7/hello/func_tests)。密度复验:coremark join −1.500487%%(276,101,561,774→271,958,690,988,与 codex −1.50049%% 数字级一致),收益严格闭合 move 桶(−4,142,871,416),matrix 双环 0x402d58 29→25/0x402df8 47→45 逐字;smallpt OFF 149,469,573,869 逐字 ON 逐字 spill 零增;zip7 OFF 逐字 −1.08125%% spill 零增;c-ray −0.14387%% spill −126(codex +6,皆 1e-7 噪声);STREAM 密度≈0 spill 0→0。静态口径修正:codex"RE=1 +100B 静态税"为覆盖噪声口径,合并方公共 PC(619 units)实测 −804B——静态实为小胜。墙钟:coremark 7 对交错 −2.4766%%(codex −2.4499%%);STREAM 四核 7 对交错全退 Copy −3.556%%/Scale −1.968%%/Add −2.193%%/Triad −3.338%%(27/28 kernel-pair 同向,codex +1.38%% 同结论)——密度≈0 但带宽四核同退,继 lazy flags Scale 之后第二例近零密度大墙钟效应(Apple 调度/placement 敏感家族)。裁决:合并 OFF 作证明基座,不翻默认(收益 −1.5%% 远低于 6-9%% 预期+STREAM 回退双杀)。未兑现根因:严格证明排除两类大形态(LoadMemory→SignExtend→ZExt→BitExtract 不能沿固定家提前发布;Mul/Add→ZExt 多家发布与 W-α read reuse/tie 同家交叉),实验性放宽被 emitter ASSERT 实抓(fail-loud 再验证)。重开前置:一 SSA 多 fixed-home 发布需不可变 component owner+W-α write/read 统一事务式决策。orb Linux 门禁全绿:build、suite 双态(196 cases)、双分片×双态指纹逐字(ON 亦逐字)、helper-fault 38/0、oracle(coremark 0x4983、STREAM Validates、sqlite TOTAL、hello 42、func_tests 101)。region branch-only flags 翻盘 ON 收官(merge `0c688d1` + flip `3a654f9` + linux golden `e76c9f6`,region-branch-report.md):coremark 定因候选②交付 +568/−42 七文件;机制复用 BackedgeFlagsPlan 加 dead_successor 模式(不造第二套 lazy 状态)——恰一条 flags-dead 内边:目标块在任意 observer/fault/control split 前完整覆写六 flags 并经 AdvancePC 提交,热边只携 host NZCV 过 terminal branch,cold/live 边 veneer 先物化 polarity/NZCV/PF/AF;U8/U16 窄 Sub 白名单 PF/AF 延迟,cold 臂从架构家在 x11+死 x16/x17 上重算(不延长热 SSA interval),PlanRegionBranchPFAF/ReproveRegionBranchPFAF 双闸 ASSERT fail-loud;全出口闭合(fault PC 前 committed 一致、region 外边/direct link/dispatcher/CallHost/signal poll cold stub 全物化、CFINV 两分片精确 polarity、与 flags_loop_lazy 互斥不两提交)。合并方 master build 全门独立复验:diff 七文件精读;suite 双态 PASS;OFF darwin 双分片逐字(ON 预期分叉:1781→1786 units,dead edge 处 unit 分裂,33 unit-line 我侧逐条吻合);helper-fault 38/0;alarm 18/18(RE0L0 负对照 rc=124);oracle 双态全绿(openssl sha256 完整表 rc=0 已入模板)。密度复验:coremark join −3.5709%%(276,101,562,901→266,242,350,777,digit-level 吻合),收益逐桶闭合(AdvancePC merge −4.838B、窄 Sub PF/AF −3.464B、polarity store −1.210B、polarity LoadImm −0.845B);七语料动态全降(smallpt −0.461943%% digit-exact、c-ray −0.229%%、zip7 −0.252%%、sqlite −0.416%%、openssl −0.55~0.59%%、STREAM ≈0),spill 全零增;静态 host_static 口径(与 hoist 翻盘同口径)七语料全降。口径警示:svm-unit host bytes(含 cold veneer+对齐足迹)七语料 +1.2~8.2KB 全升——codex 只报热路径口径,veneer 足迹如实记录在案。墙钟裁决(合并方):coremark 7 对 7/7 同向 −3.3093%%(强于 codex −2.5846%%);STREAM 四核 7 对中位 ≤1%% 方向混合,且核环 0x401ba0(Triad)/0x416980(Copy)实证不在 eligible 集(101 个 eligible 全在冷/驱动码),不触发宽度链式 NO-FLIP;zip7 5 对 +0.40%%(4/5 偏慢 sub-1%%,密度 −0.25%%,留作翻盘后观察项)。翻盘由合并方执行:指纹 corpus 首次因翻盘改变默认发码,darwin/linux 四分片 golden 全部重生(1786 units);orb Linux 翻盘门禁全绿(suite 双态 197 cases、双分片默认逐字、helper-fault 38/0、oracle 全绿)。回退=SVM_FLAGS_REGION_BRANCH=0。zip7 定因收官(纯研究零代码,基线 `aa197c2`,zip7-gap-report.md):主矛盾定名为32 位整数值在 fixed-home 发布/宽度转换/地址消费间反复搬运——move/宽度桥 42.362%%(七语料最大池),boundary gross 15.044%%(供给账非可删池),uniform 5.284%%,flags 5.199%%(剩余为 sbb/cmov/select 值化 predicate,非已 ON 的 compare→branch),spill 0.164843%% 但 95.181%% 集中单 PC 0x427fea(非主攻)。互斥责任账精确闭合:move-producing 36.187%%+boundary hot-path 13.353%%+work 40.023%%+uniform 5.238%%+flags 5.199%%=100%%。FEX orb 运行时真取证(FEX_BLOCKJITNAMING perf-map+kill -STOP 抽 /proc/pid/mem+objdump+post-RA IR CodeBlock 顺序映射,五对象 4/4、16/16、45/45、245/245、3/3 严格相等):四条热环字节级配对闭合 19.121B=SVM host 6.0735%% 实测下界。头号实锤 0x5b28d0(128× add/xor 校准环):SVM 398 条 vs FEX 133 条,SVM 255 条 lsr #0 空桥 vs FEX 128 个算术 op 各 1 条——合并方复验:RE=0 entries 31,999,993×RE=1 shape 398 逐位一致、move_static 262、spill 0x427fea 493,961,598 逐位、FEX 抽码零 lsr #0。orb 无 perf,全程序 FEX retired 数标注为墙钟反推不冒充硬件计数(0.674 锚→总差 102.63B,四环覆盖 18.63%%)。候选表:①SVM_RA_WIDTH_CHAIN_LONG 长链 ownership 合并 GO(收窄组件长度阈值长直链,不重开 STREAM 罚单的全形态 width-chain;验收三硬点=0x5b28d0 的 255 桥消 ≥90%%+STREAM 四核公共 PC 发码逐字不变+sqlite/fault/alarm fail-closed;保守 3-5%% host)②SVM_EA_FIXED_REG 条件 GO(1.5-3%%,已 pin GPR 直接 register-offset 寻址)③SVM_FLAGS_PREDICATE_FUSE 条件 GO(1-2%%,值化 DAG 不重复 region branch)④SVM_REGION_HOT_TIER 条件 GO(1-3%%,profile 二次编译扩 budget,FEX 245-CodeBlock object 为结构证据;静态 eager 扩窗 NO-GO)⑤spill remat <0.12%% 低优先;全量 pin/跨 unit flags/XMM 扩驻留/lazy 借壳全 NO-GO。无新正确性 bug。zip7 候选①长链 ownership 合并翻盘 ON 收官(merge `802e2f9` 缺省 OFF + flip `7149fd7`,width-chain-long-report.md):zip7 定因候选①交付 +348/−7 五文件——新 B 类 SVM_RA_WIDTH_CHAIN_LONG,同一 pinned fixed home 上 ≥32 步连续 U32 Add/Xor→ZeroExtend32To64→SetHostGPR 发布链建立完整拓扑证明后,放开旧宽度链对 W-α 合并发布点输入的保守禁令,省掉低 32 位 BitExtract identity 快照;链内只允许 AdvancePC/BitExtract/LoadImm/ClearFlags/SaveFlags,memory/atomic/helper/call 与其他 producer 一律截断,8/16 位 partial write 不进白名单;第三方同家 interval 双侧相交仍拒绝,唯一例外为精确末次 use 的 producer 接管;fixed clobber/异源 SetHostGPR/DirtyGPR 逐桥回放 CheckInstr 失败恢复;emitter 独立复证唯一 U32 Add/Xor consumer fail-loud ASSERT;与旧 SVM_RA_WIDTH_CHAIN 独立(其缺省保持 OFF),新增独立门防止 LONG 单开时旧通用池生效。合并方 master build 全门独立复验:diff 五文件精读;锚点三硬点全过——0x5b28d0 RE=1 shape 398→144/move 262→8 与报告逐位一致,RE=0 entries 31,999,993 两态相同,动态闭合 254×31,999,993=8,127,998,222 精确到条,region root 0x5b28c0 原始 blob 3436B→1404B、指令字级 lsr w,#0 计数 514→6(两形态含 seed 各 1+4 残余)、add/eor 算术条数两态恒等;STREAM 硬门 2539 公共 PC 三字段+544 unit 足迹字节零 diff(独立门生效实证);coremark/smallpt/c-ray/sqlite/openssl 公共 PC 零机制税(coremark 4 冷 libc PC 为覆盖分组噪声、unit 字节净零),spill 全零增。密度复验:zip7 host 314,828,530,310→306,700,530,368(−2.5817%,ON 侧与报告逐位一致)、move −6.0944%;smallpt 两态逐字。墙钟(合并方交错):zip7 5 对 4/5 同向、对差中位 −1.75%(机器窗口噪声大于 codex 窗 −2.88%,方向量级一致);coremark 7 对 +0.06% 中位 parity(零 eligibility 正确结果);STREAM 四核 7 对方向混合 ≤0.67%,叠加发码恒等硬证裁定非回归。门禁:套件双态 197 cases、darwin 指纹 OFF 双分片逐字+ON 双分片逐字(0/1786 unit 变化,golden-stable 翻盘实证)、helper-fault 双态 38/0、alarm 18/18+RE0L0 负对照 rc=124、oracles 双态全绿(含 openssl sha256 完整表)。翻盘由合并方执行:golden-free(指纹 corpus 无长链形态),翻盘后默认态 darwin 双分片逐字、套件/helper-fault 复跑绿;=0 回退态指纹逐字。orb Linux 翻盘默认态门禁全绿(suite 双态 197 cases、linux 双分片默认逐字、helper-fault 38/0、oracle 全绿)。回退=SVM_RA_WIDTH_CHAIN_LONG=0。zip7 候选②EA_FIXED_REG 设计门前审计 NO-GO(零代码收官,基线 `f104025`,ea-fixed-reg-report.md):阻塞=mac 运行模型与 A64 编码的机制矛盾——bounded-bias 正确地址为 x24(pt)+zext32(base+index×scale+disp) 三寄存器项+modulo-32 回绕,A64 register-offset 只有 Rn+Extend(Rm)<<S 两项且无 disp 字段;以 base 为 Xn 漏 pt 确定性错址,以 pt 为 Xn 仍需先物化 full guest EA(现状),预计算 pt+base 越本单"不占新 pin/不新增长活 interval"授权。mac 无条件 bounded-bias(main.cpp 注释:4GB pagezero 阻止低 ET_EXEC identity,SVM_MEM_IDENTITY=1 实测仍 identity=0 bias=0x300000000,合并方 master 二进制复核逐字一致)。两锚点严格 eligible 命中=0:0x42d0b0 毛上限仅 0.4322%(3 条 GetOperand×441,868,158),0x41e148 毛上限 0.2775%,合计 0.7097% 且全是"假设三寄存器可编码"的极宽口径——address gross 7.758% 是定位 lens 非可删池。合并方独立复核:own master build 密度数据锚点 shape(14/3、150/78、entries)与报告逐位一致,mac mem-mode trace 复核一致,tracked diff 为空,基线门禁当日已在同代码全绿。备选形态全否决(pin 改 host pointer 毁 guest 语义/helper/fault ABI);identity-only scalar Plus(value,value) 残余为 Linux 另案(PreserveMemoryEA 已覆盖 base+imm 与部分 PlusExt,池需 orb 重量,对 mac 侧 campaign 比率零贡献,暂不立项)。下一候选③predicate 值化 fusion 已派出。zip7 候选③PREDICATE_FUSE 审计 NO-GO(零代码收官,基线 `f104025`,predicate-fuse-report.md):池为真但 proof carrier 未解耦——值化 predicate 毛池实测存在(放开 Adc/Sbb 的 flags backward DCE 后 987 公共 PC 毛省 6.497B=2.118%,两锚点 0x42d710/0x42d000 合计 2.824B=0.921%,消费者本体已是紧凑 cset/csel/sbc),但 SaveFlags 同时承担三种职责:架构 flags 写入、region/backedge plan 的证明输入(PlanBackedgeFlags 扫描 final save)、RA fixed-home/width-chain 的 observer(IsPinnedCoalesceObserver)——DCE 它即拆已翻盘机制的证明结构。原型 A(per-guest-region liveness+observer 屏障)净 +18.679% 回退(0x41e148 150→282、0x42d0b0 14→25);原型 B(保留 terminal proof 尾部)更坏:0x5b28d0 从 144 膨胀到 1560(+82.014%),证明 RA observer 是独立阻塞面。2×2 矩阵证非重叠:region-branch 收益在原型中仍保留 0.585B,问题不在收益重叠而在 IR 证明载体被改。合并方独立复核:锚点 OFF shape(134/51、88/32、entries)与自有 master 密度数据逐位一致,FlagsEliminationPass 遇 Adc/Sbb 整块 return 的保守注释与 IsPinnedCoalesceObserver 含 SaveFlags 均源码实锤,tracked diff 为空。重开前置=独立 FlagsRecipe/ProofToken 新 IR 设计(FlagsElimination/RA/backend plan 共同消费,不被 DCE),且 0x5b28d0 width-chain 逐字不变设为第一道设计门——非 spike 级局部 fusion。下一候选④REGION_HOT_TIER 已派出。zip7 候选④REGION_HOT_TIER 设计门审计 NO-GO(零代码收官,基线 `72dff30`,region-hot-tier-report.md):生产实现前冻结预算(cap64 动态毛 ≥0.75%、翻译/字节税各 ≤5%、0x5b28d0 逐字不变),实测仅最后一项过——离线 cap 扫描:cap32 −0.169%/cap64 −0.449%/cap127 −0.246%(不单调),cap128 触发 lazy_budget==kMaxFuncBlocks 语义悬崖(lazy=false 失跳过已发布 successor,host +17.316%、0x5b28d0 144→281)非同一机制;cap64 翻译 +11.14%、发码字节 +6.76% 双超预算;不可能实现的逐 PC oracle(只挑变短 PC、零回退零二次编译税)毛上限仅 0.642% < 冻结门。热点判据死结:FEX 锚点 0x42ca90 root 仅 4,308 entries(内部 PC 数千万,RE=1 内部入口不过 root counter),阈值 5,000 漏标、4,308 同时选 22 unit,新增内部计数=给热路径加永久税;替换协议不存在(Module::Push insert-only,无 compare-and-replace;L1/L2/direct-link/fault/QSBR 原子替换需新事务,SMC 事务不能直接复用);JIT cache v5 无 tier 元数据。合并方独立复核:cap128 悬崖源码实锤(translator.cpp:732 lazy=lazy_budget<kMaxFuncBlocks);自有 master build cap64 复跑 host=305,322,882,375,与自有 cap16 基线差 −1,377,647,993 与报告 digit-exact(−0.449183%),0x5b28d0 cap64 下 144/8 第一设计门逐字通过;tracked diff 为空。重开线归档:需独立安全 OSR/code replacement 基建+无热路径税的内部热度采样,且离线 oracle ≥1.5% 才再审。zip7 定因候选清单收官:①长链翻盘 ON(−2.582%)②③④审计 NO-GO ⑤spill remat <0.12% 低于行动线不立项;下一=harness 全环境重基线。baseline8 全矩阵收官(2026-08-09,REPS=5 warm,results/baseline8/ + coremark 补跑格 baseline8cm):锚 baseline7(08-07)之后两波翻盘(region-branch 3a654f9、width-chain-long 7149fd7)计入实测——**coremark 0.509→0.541**(SVM 14,931→15,547 iter/s = +4.13%,与 −3.57% 动态+累计叠加吻合,FEX 侧 29,340→28,735 近似) Campaign 新高;**Triad 0.563→0.616** 新高,Copy 0.997→0.975/Scale 0.338→0.333/Add 0.454→0.446;smallpt 2.203→1.985(SVM 11.239→10.478s);cray 2.322→2.049(SVM 28.98→25.88s);sqlite 1.238→1.185(SVM 绝对 12.360→12.061s = −2.4%,比率口径仍只看 SVM 绝对值);zip7 0.674→0.664(SVM 4134→3910、FEX 6133→5889 同窗双慢——交错 A/B 当窗 4/5 对 −1.75% 才是机制证据,跨窗比率落于历史漂移带 0.619-0.699 内,不作回归读);osslsha 2.561→2.533 胜场保持,osslaes 0.913→0.891。STREAM Copy/Scale/Add SVM 侧 −3.6~−4.2% vs Triad +7.7% 方向混合——两翻盘对 STREAM 一发码零 eligibility 一逐字节恒等(硬证在手),判窗口噪声。oracle 23/24 + coremark svm 格因提速再触 10s 下限(150k 迭代 9.993s)判 invalid——校准已提 175k(bench 仓 `26fdc4c`),补跑格 validated 15,547.26 入账。跨窗墙钟与机制收益的归因纪律不变:机制账以同窗交错 A/B 与密度为准,baseline 间差值只作大势参考。master = `3cd6a62`。 %% 2026-08-09 STREAM 候选⑤ XMM0_LOOP_PROMOTE 审计 NO-GO 收官：供给门通过(自然 FPR 池 21 钳 19 全程序 host +451 条/+0.0000013%、spill 0、max live FPR 7 不变),但"相邻动态迭代不同 host 家"在单份 block 发码+单份 RA 映射+单 recovery fault 元数据下不可表达——回边换名本质是 loop body A/B clone,需第二 RA namespace 与 per-fault-PC phase recovery(input/RHS/output 三 fault 阶段分别取上一缓冲/当前中间值/当前终值,signal poll 与正常退出取当前 phase),缺一项即在 input fault 或 Triad RHS fault 发布错误值,硬做重开 24f369e 同类陈旧快照漏洞,设计门 fail-closed 零代码收官。现价毛池修正为 5 条/迭代(Scale 相邻同值快照已被 UniformStoreSink DSE 精确删一条)=1,999,999,900 条 5.796100% host dynamic,理论天花板据此下修。合并方独立复核:tracked diff 0;代码主张逐点成立(Translate 单遍 :806/:1197/:2184、loop_hoist_body_entry 单 label :822/:1022/:1814、FaultEntry 单 recovery 无 phase 元数据 module.h:47-57、recovery_offset 每 block 一份 :1791-1801、CaptureLatestSnapshots :233、RA 单 MapRegister :2643-2706);锚点形态以自有 master 构建 STREAM 数据逐字相符(三环 host_static 28/32/22、w16 snapshot store 1/2/2、Scale 热路径 24 条),算术闭合 5×399,999,980=1,999,999,900。重开前置(不可借本开关混入):先独立建设 per-fault-PC cold state recovery 基建(fault 元数据描述 host PC 范围→从哪个已证明仍活的寄存器重建哪个 State slot,覆盖序列化/SMC/QSBR/signal/fault 测试),再做通用 self-loop A/B body clone + 第二分配 namespace,此前五条热快照 store 不得删除。至此 STREAM 快照线与 zip7 定因候选全部收官(长链翻盘 ON,EA/predicate/hot-tier/XMM0 四连设计门 NO-GO)。master = `5c9cbec`。 %% 2026-08-09 STREAM 定因 round 2 研究型 NO-GO 收官:Scale/Add 的 3× 差距归因落定——不是 FP 本体(FEX 同样发 ldr q16;fmul v16;str q16 同名链仍达 83.8GB/s),而是单元素 body 后每 16B 一次的串行迭代边界:packed-flags 提交 11 条(Scale/Add 两核 23.0331% host 毛池)+ LDAR/CBNZ/B signal 观察链 3 条(三环 9.4226%)+ 回边,压死 outstanding work/MLP。guest census 我逐字复核:Copy 是 memcpy_ssse3 5 路展开+movntps(80B/迭代,43 条摊厚),Scale/Add/Triad 仅 16B/迭代。xctrace CPU Counters 四环对照:Copy 1.149 vs Scale 3.772 cycles/16B;固定 guest PC 的 Scale K=8 展开实验 wall −55.1%/PMU cycles −52.3%,合并方用 w66 二进制独立复跑 K1=4.84-4.87s vs K8=2.25-2.31s 坐实;K2/K4 反而 +39.4%/+35.8% 退化,证明机械 unroll 踩 Apple 代码形状雷区,不能直接转生产 pass。tracked diff 0;毛池算术复核闭合(11×2×399,999,980=8,799,999,560=23.0331% 等)。PMU 能力缺口明示:无 retired-ins/L1D/L2/store-buffer 计数(powermetrics 需 sudo,xctrace 模板只给 cycle 与四类分量)。唯一新方向 SVM_LOOP_SUPERUNROLL 裁为条件研究/当前 NO-GO 实施——毛池与已否的 lazy-flags/zero-tax safepoint 重叠不可重复记账;重开第一门写死:保持全部 fault PC/flags 语义的 K8 原型 Scale wall ≥20% 且 K2/K4 反噬消失,否则关闭。至此 STREAM 两轮定因共 14 个候选/方向全部收官(2 翻盘、1 DSE 落地、11 否)。master = `1834e86`。 %% 2026-08-09 sqlite 定因收官(零代码,sqlite-gap-report.md):真实差距 1.19-1.24 稳定(baseline6 翻胜证伪为 FEX 窗口噪声),且生产口径被 IO 掩盖——同机 SVM 文件库/:memory: CPU 比率实为 1.865,文件 IO 把表观比率压到 1.052;FEX 共享 /mnt/mac DB 的 sys 5.85s 是 OrbStack 共享层税不可比。分解:join host 123.8B,move/宽度桥 31.48%、flags 6.50%、GetOperand/EA 7.13%、boundary 10.45%;热度极分散(top20 仅 18.46%),单点修复不解 1.22×。FEX /proc/mem 实码对照九热点整块差额 6.063% host,闭合到两类可泛化形态:①bounded16 截断热闭环——0x4a5040 恰 16 block,0x4a5358 回跳目标被截付 26 条 vs FEX 4 条,单锚点 1.2332%;②16-bit rotate 展开 17 条 vs FEX bfi/ror/bfxil,三锚点子链 0.5255-0.6832%。合并方独立复核:tracked diff 0;自有 build 双臂重跑 entries 逐字(0x4a5149=98,206,673 等六锚点全符)、0x4a5040=16 block 与 0x4a5358 external=1 坐实、move 31.4748%/spill 0.1989% 对符;代码主张(RegionFuncBudget 1→16、Lsl/Lsr/Ror 单节点 lowering、Sse42Str 预算 {4,4})逐点成立。另发现独立 opt-in 缺口:SVM_SSE42_STRING_INLINE=1 跑 sqlite 确定性 rc=134(scratch GPR declared 4 实需 5),我复现 digit-exact——该开关在修复前 NO-GO。派出两候选:①SVM_REGION_CYCLE_CLOSE(cap-hit 最小闭环替换,条件 GO,第一门全局 cap-hit  census+供给税预算),②SVM_NARROW_ROTATE_COMPACT(白名单 U16 rotate DAG 紧凑 lowering,GO,第一门三锚点逐位差分)。弱化 F_FULLFSYNC(改 durability 语义)与 memcpy spill(<0.2%)明确 NO-GO。master = `359e5a4`。 %% 2026-08-09 sqlite 候选① REGION_CYCLE_CLOSE 第一门审计 NO-GO 收官(零代码):fail-closed 原型的七语料 census——sqlite 全程序 cap-hit 闭环毛池 602,268,247=0.486463% < 写死的 0.5% 门,净额仅 0.293036%;七语料无一过门;STREAM 零需求却 +2,544B(+0.609%)unit 足迹供给税。关键修正:定因锚点旧账 22 条差额实为 source 26→18 仅省 8 条(target 0x4a5138 因 SCC observer 16→18 付 +2),锚点净 360.9M 而非 1.526B——第一门正是为此设立。合并方复核:tracked diff 0(原型逐 hunk 撤除);ON/OFF 臂日志坐实 26→18/16→18;算术闭合(69,401,516×8=555,212,128、97,158,681×2=194,317,362);结构性发现入账:x86 无条件 direct jmp 解码为 SetLocation+ReturnToDispatcher(decoder_control.cc:9-18),不产生 LinkBlock successor,普通 region BFS 天然无法闭合此类边;HIRFunction::RemoveEdge 是空实现(hir_builder.cpp:251)——未来任何 speculative CFG 形成须先单独建设有测试的 edge rollback 基建。扩大 extra=2 与同预算踢叶两扩展均无账支撑不实现。sqlite 候选队列余:②NARROW_ROTATE_COMPACT。master = `118a368`。 %% 2026-08-09 sqlite 候选② NARROW_ROTATE_COMPACT 翻盘 ON(机制 `ebf4cba` + 翻盘 `0eb1419`,回退 =0):U16 立即数 8 rotate 数学恒等 bswap16(ROL16(x,8)==ROR16(x,8)==REV16),白名单仅 register 目的地+imm&31==8,CF/OF 原公式不变,emitter U16 复证 fail-loud,memory 目的地负测零命中。合并方 master 构建全链自验:三锚点 −13 条 digit-exact(82→69/40→27/78→65),sqlite 动态 Δhost=−1,595,226,919 与报告逐字一致(闭式 122,709,763×13),spill/uniform 精确不变,六语料公共 PC 双口径零差;套件双态绿;OFF/ON 指纹 darwin+linux 四分片全匹 golden(corpus 无此形态,golden-free);helper-fault 双态 38/0;alarm 18 格全 rc=0+负对照 rc=124;oracle 双态全过。墙钟:codex 交错五对 −1.93%/−2.05%,我复跑 TOTAL 中位 −1.04% 同向——sqlite IO 掩盖 CPU 差(定因已证 :memory: CPU 比率 1.865 vs 文件库 1.052),密度 digit-exact + 同向交错即机制证据。2,048 组 JIT↔Unicorn 差分(ROL/ROR×count 0-15×64 flags 组合)双态过。sqlite 候选队列清零(①第一门 NO-GO、②翻盘);matrix 现存最差格:c-ray 2.049/smallpt 1.985(定因残余为结构性)、osslaes 0.891。master = `0eb1419`。(2026-08-09) SSE42_INLINE scratch 修复+翻盘 ON:根因=VIXL 双 lane Movi 宏(macro-assembler-aarch64.cc:1127-1134)在 Sse42Str 已声明 4 GPR 全租出后再隐式 AcquireX,硬门 jit_context.cpp:1219-1220 正确 fail-loud(rc=134, 合并方 digit-exact 复现);修法=emit_u128 lambda 复用已声明 scalar 临时(Mov lo/Ins lane0/Mov hi/Ins lane1)替换 4 处双 lane 常量站点,预算 4 不涨价(否决 4→5——会永久压缩 RA value GPR 池);合同测试全 256 imm8 形态断言 LastInstructionScratchGPR()==4。修复后价值重估翻盘:sqlite join 动态 −441,086,325(−0.360905%,58 PC,锚点 0x506d74 168→127 =−41,4,550,732×41=186,580,012 逐条闭合),生产墙钟 5 对交错中位 −2.453% 全同向(合并方复测 −3.24%);其余六语料动态中性(coremark +39/smallpt 0/c-ray −6,393/zip7 −302/STREAM +46/openssl −15,511),spill 逐字不变;PCMPxSTR 全模式 Rosetta+SDM 差分双态 16,255 断言全过;指纹 4 态逐字 golden(corpus 无此形态,golden-free);helper-fault 双态 38/0;alarm 18 格 rc=0+负对照 124;orb Linux 全绿(suite 双态 1,043,034 断言、linux 双分片指纹 golden、helper-fault 38/0、oracle 全套)。回退 SVM_SSE42_STRING_INLINE=0。FeatureSet 62→63、env 152→153。sqlite 定因候选队列至此全清(①CYCLE_CLOSE 第一门 NO-GO、②NARROW_ROTATE 翻盘、④SSE42 修复+翻盘);matrix 最差格剩 c-ray 2.049/smallpt 1.985(结构性残余)、osslaes 0.891(唯一未定因格)。master = `363c46f`。(2026-08-09) osslaes 0.891 定因收官(研究,零代码):非弥散——top-1/5/20 unit 覆盖 16.05%/48.06%/64.72% host 动态(总账 619.6B:move/bridge 26.59%、boundary 15.53%、flags 5.67%、EA 3.39%、spill≈0)。AESENC/PMULL 两侧均原生指令,差额在三处形态:①六路 AES destructive producer 链未绑 XMM 固定家(入口 copy+出口 publish 12 条/entry,FEX 零 copy 直写 v18-v23 驻留家,/proc/pid/mem 抠码 digit-exact)→候选 SVM_RA_AES_CHAIN_TIE 闭合 22,653,991,500=3.656103%(三锚点 0x760790/0x7607db/0x761480 各 12 条×entries);②pshufd $0x4e 降成 10 条常量物化+TBL,而 [2,3,0,1] = EXT #8 一条(同 unit 已有 ext #8 实例)→候选 SVM_PSHUFD_4E_EXT 闭合 12,337,106,410=1.991072%(三锚点各省 10 条,严格 imm 白名单);③AESKEYGENASSIST 19 条 vs FEX 8 条(swizzle 常量 10 条物化 vs State 只读 named-vector 表 1 条 ldr;rcon zero+FMOV+INS vs DUP)→候选 SVM_AES_KEYGEN_COMPACT 闭合 3,637,096,254=0.586987%,条件 GO(须先过只读常量表基建门:code-cache sizing/disk revive/State 布局,禁走已否的 deferred literal pool)。三候选零重叠合计 38,628,194,164=6.234162% host 动态下限。合并方复验:11 个 RE=1 形态锚点自建 master 构建 digit-exact(86/80/158/65/40/60/54/55/169/69/32),keygen 19 条区间逐条核到,FEX 双侧抠码形态 digit-exact,0x4e→EXT#8 恒等式数学验证,tracked diff=0。不立项项:AESENC 本体(3/2 条池 0)、zero reuse(两侧各 1)、PMULL(两侧同形 4+2+1)、一般化 flags/boundary 删除(0x760790 余 23 条含 committed flags+poll+ABI,不可借壳复活 W-β/W-η)、一般化 literal/ADRP。追平需 SVM 吞吐 +12.23%,6.23% 是指令下限非墙钟承诺。matrix 全部格完成定因。master = `f26670c`。(2026-08-09) RA_AES_CHAIN_TIE 翻盘 ON(osslaes 候选①,机制 `734f376` + 翻 `952614d` + 测试基建修 `dc9f60a`,回退 SVM_RA_AES_CHAIN_TIE=0):unit 内连续 VecAesEncFast/LastFast(≥2 节点)整链绑 resident XMM 家——链首 copy+链尾 publish 各 6 条归零;关键不变式=每个中间 AES 节点本身就是 guest committed XMM 写,fixed home 从首节点完成起恒持最新值,fault recovery 经 BuildSaveStaticUniform 取 home 即正确,故 observer 白名单只放行普通 LoadMemory/StoreMemory/SaveFlags,其余 fail-closed;emitter 每节点复证 result==data==target+拓扑+单 use,ASSERT fail-loud,未标记逐字 ORR fallback。兑现:三锚点各 −12 条/entry digit-exact(0x760790 80→68/0x7607db 60→48/0x761480 158→146,冻结 entries 闭合 22,653,991,500=3.656103% host 下限),另两 AES unit 各 −6;合并方自建 master 构建复验:锚点 digit-exact、六语料公共 PC (host,move,spill) 零 diff(coremark 2897/sqlite 22048/c-ray 12255/zip7 8577/smallpt 3014/STREAM 2539)、spill 301→301、uniform/boundary 逐字不变;套件双态 全绿(darwin 1,040,593/1,040,604,linux 1,043,205/1,043,200);指纹 darwin 4 态+linux 双分片逐字 golden(corpus 无 AES 链形态,golden-free);helper-fault 双态 38/0;alarm 18 格 rc=0+负对照 124;oracle 全套双态(smallpt 3603…/c-ray BMP a2e4…/coremark 0x4983/sqlite TOTAL/openssl 六段/hello 42/func 101)。墙钟:openssl aes-128-gcm 16KiB 交错 7 对中位 +1.61% 全同向(报告方 +1.72% 6/7)。ON 臂同窗口 entries +5.3%(定速测试吞吐自证)。**翻盘后 orb 抓到一个测试基建 bug——非机制 bug**:两相冲突用例初始分配跟随编译期默认,翻盘后第一相误 tie、第二相拒绝无法回滚标记致恒假;修为初始相显式关 tie(dc9f60a),生产单相路径不受影响。FeatureSet 63→64、env 153→154。未做:AESDEC 族/单节点 tie(净零)/helper·TSO 窗/跨 block 组件,reopen 须逐 opcode alias 证明+独立池。osslaes 余候选②PSHUFD_4E_EXT 1.991%、③KEYGEN 0.587%(条件)。master = `dc9f60a`。(2026-08-09) PSHUFD_4E_EXT 翻盘 ON(osslaes 候选②,机制 `2a33df8` + 翻 `1539e3d`,回退 SVM_PSHUFD_4E_EXT=0):pshufd $0x4e 选择子 [2,3,0,1]=64 位半交换,恒等单条 ext Vd,Vn,Vn,#8(数学验证+同 unit 已有 ext 实例);识别放 RA 分配后只读标记阶段(前端看不到 vec_const_cache 合并后的全 use 集),白名单=VecLoadConst 逐位 == (lo=0x0f0e0d0c0b0a0908, hi=0x0706050403020100) 且全部 uses 均为同 mask VecShuffle32Indexed 的 arg1(常量作 source/混用 consumer/跨 block·terminal use 计数不等 → 整体回退);emitter 独立复证 mask+use 集+marker,ASSERT fail-loud;命中常量零发射+TBL→EXT,未命中逐字原路径;不引 literal pool/ADRP/State 常量表。兑现:三锚点各 −10 条/entry digit-exact(0x634960 86→76/0x634b15 69→59/0x634260 169→159,冻结闭合 12,337,106,410=1.991072%),另 0x6345cf/0x634901 同族各 −10;0x634260 同 unit imm=0xff 保持原路径实证。合并方自建 master 复验:锚点 digit-exact;六语料热 PC 零 diff(仅零 entries 冷 root regroup);spill 301→301、uniform 逐字不变;套件双态(darwin 1,047,574/1,047,565+翻盘默认态 1,047,549 亲跑,linux 1,050,152/1,050,150);PSHUFD 专项 6,950 断言(全 256 imm×alias×memory RHS×4 位型×双态,数学 lane selector golden);指纹 darwin 4 态+linux 双分片逐字 golden(golden-free);helper-fault 38/0;alarm 18+负对照 124;oracle 全套(coremark 0x4983 双态——注:裸 `coremark_x64 200000` 受时间帽影响迭代数不定,canonical 参数为 0x0 0x0 0x66 200000 7 1 2000;smallpt 3603…/c-ray a2e4… 双态)。墙钟:aes-128-gcm 16KiB 交错 +1.30% 6/7(报告方)/+1.71% 5/7(合并方,窗口热漂移可见)。FeatureSet 64→65、env 154→155。osslaes 余候选③KEYGEN_COMPACT 0.587%(条件 GO:须先过只读 named-constant 基建门——code-cache sizing/disk revive/State 布局,禁 deferred literal pool)。master = `1539e3d`。 候选③ KEYGEN_COMPACT 收官:只读 named-constant 基建评审 GO——落点 Runtime State 前 16B 语义只读前缀(`ldur q,[x28,#-16]` 单条,State ABI/uniform/磁盘 cache/fault-signal 零扰动;deferred literal/ADRP/RO 页/trampoline 尾维持否决;通用 VecLoadConst 转表普查全语料毛上限 <0.05%,基建窄服务 KEYGEN)。机制默认 OFF 合并:swizzle 10→1 条 + rcon 4→2 条(Mov+Dup),整点 19→8 条;九锚点合并方 build-master 复验 digit-exact 41→30 −11/entry,冻结闭合 3,637,096,254=0.586987%;六语料公共 PC 零 diff(coremark 仅 6 个零 entry 冷 root 分组噪声),spill 301→301;mac+orb 套件双态、指纹四态 golden-free、helper-fault 38/0、alarm 18+负对照 124、oracle 全套双态绿,warm revive 双态一致。墙钟双独立窗口中性(−0.11% 3/7 与 −0.42% 3/7),**不翻默认**;SVM_KEYGEN_COMPACT=1 保留,重开条件=独立窗口中位为正且多数同向。osslaes 定因三候选全收官(6.234% 下限:①3.656% ②1.991% 已翻盘兑现,③0.587% 机制备便未翻),FeatureSet 65→66、env 155→156。另录:解释器 RunVecAesKeygenAssist lane 排列与生产 JIT 不一致(既有问题,非本机制引入,专项以独立数学 golden 双态逐位验证),留作独立正确性审计项。master = `f149462`。 STREAM super-unroll K8 全语义原型研究收官(**NO-GO,零代码**):真实 stream_x64 主程序离线 ELF 原型(code cave 重定向四热环,每复制体保留原 induction/CMP/条件出口,四核 80 次测量全 Solution Validates)——K8 Scale 仅 +0.244%(GO 线 ≥20%),Add +0.351%,Copy +2.681%,Triad −6.578%;join 账 K8 全程序 host 反 +1.4396%。钉死机制:round2 的 K8 −55.1% 来自把 CMP+flags 提交+branch+poll 摊薄到每 K 次,而语义保持原型每 body 仍须精确提交,尾链税未被摊薄(Scale 每 16B 仍 26 条,K×(400M/K)×26 与 K1 逐字相等)。K2/K4 反退化定因:Apple 后端对同名 V 家短链/WAW 强惩罚(PMU processing 67.35%/71.87%),换名对照 K2 7.08s→4.23s、K4 6.90s→2.51s——生产形态须 clone-aware RA 换名,不能借 guest XMM。fault 专项(8 断言)证实现有 distinct-PC recovery 正确,缺的是同 guest PC K clone 的 per-instance recipe(FaultEntry/BackedgeBlockMetadata 单 guest_loc 单 recovery 表达力不足,已逐行复核)。重开条件(须同时):同 PC 多 clone fault 逐位恢复+clone 不同 host V 家 spill=0+组尾唯一 poll alarm 格全中+真实 K8 Scale ≥20%+其余核无 >2% 确定性退化+七语料近零税。当前缺第 1/2/4 条,不合半成品 gate。master = `0153fd0`。 结构残余方向枚举收官(研究,零代码):coremark/smallpt/c-ray 现价结构账重切(move/桥 38.9%/32.0%/36.5%,boundary gross 24.1%/12.4%/13.1%,flags 4.4%/11.1%/7.1%),FEX orb 实机抠码佐证(smallpt SRA marker 不落 State vs SVM 同 root 8 条 q-load/store;coremark MADD/shifted-ALU 假说被真码否决)。唯一立项:**SVM_UNIFORM_LOCAL_SSA**——unit 内把 State home 当普通 SSA 做跨 HIR block load forwarding(store 全保留,fault/signal/exit 零扰动,不占固定家,RA 净负 per-unit revert);emit 级毛池 0.614%/1.704%/1.672%(core/smallpt/c-ray),load 半边 0.570%/0.800%/0.855%;立项门=rays 净值各 ≥0.5%(analysis-only CFG facts + RA dry-run),coremark 只作附带账。二阶段 SVM_FAULT_CONTEXT_RECIPE 条件 GO 靠后(per-fault cold veneer,store 池 0.043%/0.903%/0.817%,须先过 snapshot boundary 分类门,任一 <0.5% 即否)。白名单扩池(0.077%/0.112%)/MADD(FEX 真码证伪)/shifted-ALU(命中已否 EA)/STP 配对(0.028%)/boundary 再内部化(无新载体)均量化 NO-GO;若 local SSA 审计也不过门,则正式裁定方向空间耗尽。master = `ab8d88f`。 UNIFORM_LOCAL_SSA 第一门审计 **NO-GO + 方向空间正式耗尽**:strict 臂(生产入口契约:runtime.cpp:975-978 逐 decoded block 发布 label,direct link/RSB/code miss 可落 unit 内任意 block 边界——已逐行复核)净池 smallpt 0.048408%/c-ray 0.175374%,距 0.5% 硬门仅 9.68%/35.07%;乐观反事实(普通边传播+零 RA 税)也仅 0.0835%/0.2334%,加缺-phi 池后 0.119%/0.312% 仍不过门。池被吃层级:SetHost/fixed-home observer 截断 52-66%、外部入口清空 22-41%、provenance/多前驱不同值 9-19%、RA 窗口 7-14%。coremark 0.134881% 仅报告。按枚举报告预注册规则正式裁定:**当前证明体系内结构残余方向空间耗尽**;重开须新入口 ABI(public canonical + internal SSA 双入口)/跨边 snapshot recipe/phi+RA split 新基建之一,且均需实测 rays 净 ≥0.5%。master = `f6e983a`。FAULT_CONTEXT_RECIPE 第一门审计 NO-GO(w66 复核 digit-exact):同步 guest-memory fault 且 carrier 实发 State store 的可删池 smallpt 1,018,923,677(0.684857% host,过 0.5% 门)、c-ray 仅 661,546,454(0.144281% host,距门差 1,631,016,993 条),coremark 153 条仅报告;按"任一不达即否决、无回调"预注册规则二阶段 veneer 不立项。c-ray 被封杀的主因:82.17% store 毛池并非该同步 fault 独占 carrier(承担普通 publication/其他边界责任,不能搬入只在 guest fault 路径执行的 cold veneer),plan 存在≠carrier 发码(published_or_observable 证明已剔除 2.60B 次虚报,否则 c-ray 会被虚报为 0.7116%);A+B+C 三类逐行闭合冻结毛池误差为 0。二阶段前置事实留档:FaultEntry 无 recipe 字段、cache 格式须 5→6 bump、SMC priority-0 与 guest fault priority-100 分类链完整。至此结构残余枚举的全部预注册项收官,当前证明体系内方向空间整体封存;重开条件见审计报告 §8。master = `c8d9e2e`。coremark 双桶法医分解(move 逐条归 owner + region 形成质量)收官并落地 INT_IMM_FOLD(机制 `57650a0` 默认 OFF,本机 build-master 独立全门复核):move 桶 38.867% 零误差闭合(explicit width bridge 13.53%/算术 fallback 7.31%/publish 6.79%/LoadImm 6.22%/home load 3.95%);单 block 单用 LoadImm 折叠池保守上限 2.5549% host,锚点 0x402df8 静态 −3(42→39 gap-op 条)digit-exact、动态精确闭合 1,555,200,000 条;实测 coremark −2.559%/smallpt −1.481%/c-ray −1.748% host 动态,spill/flags/terminal/fault 逐字不变,OFF 指纹 golden 零差,双态套件+helper-fault 38/0+全 oracle 绿;但墙钟双独立窗口同向回退(codex +1.82% 7/7、本机 +1.76% 6/7,Apple code-placement 罚单再现)→**不翻默认**,SVM_INT_IMM_FOLD=1 保留,重开=可复现的 placement 机制消除回退后 coremark+STREAM 双保险重测。region 桶:coremark boundary 2 倍系 internal edge/cycle poll 密度(152,911/M guest vs rays ~41K),非外部退场贵;碎片真实(cap16+published-target)但修复手段均在封清单,无新机制。FeatureSet 67 / env 157。master = `57650a0`。INT_IMM_FOLD 翻默认 ON(`921783e`,=0 回退):按用户裁示以代码质量口径裁决(不依赖 host 空闲)——三语料 host 动态 −2.559%/−1.481%/−1.748% digit-exact、spill/flags/terminal 逐字不变、OFF 态指纹曾零差,翻盘后默认态套件 1,052,812 断言 171 用例全绿、helper-fault 38/0、双 profile golden 重生(964 行/unit 纯 IR 收缩,共 −3,206 IR)。墙钟两安静窗口 +1.8% 回退(Apple placement 假说未证伪)与一次负载风暴窗口(作废)留档;若后续 placement 机制研究解出罚单机制,本开关为首批复测对象。master = `921783e`。coremark 下一缺口审计(post-fold 基线)落地:LoadImm 余量 9,659,058,890(3.723206%)全分类 digit-exact 闭合——store operand 3,708,196,615(zero 2,302,767,683=0.887631% 可 xzr/wzr 直发,FEX GetZeroableReg 对照成立;nonzero 1,405,428,932 无 A64 store-immediate 排除)、movz/movn/movk 1.045137%(2,711,383,323 条 32-bit 常量本就需双 halfword,仅 836 条真 64-bit,"本可单条"=0,排除)、不可编码 0.408%/多用 0.146%/窗口外 0.079% 逐一量化排除;算术 operand-copy 池 7,939,437,561(3.060357%)立案为机制候选(Sub 1.8897%/Mul 0.7244%/Or 0.4463%,FEX KillMove 对照)。operand-copy 阶段 A 定因落地并自我修正:该池非"翻译层二地址预拷贝"而是 emitter span 内含 mov 的形态池——Mul 1,879,200,194(0.724361481%)系 `EmitMul` 对已是寄存器的 RHS 无条件 `MaterializeOperand`(translator.cpp:2189)再拷一条,纯浪费可删,立案 `SVM_OPERAND_COPY_KILL`(默认 OFF,emitter 源头抑制);Sub 4,902,410,790 全落窄(<32-bit)NZCV/AF/PF 路径、Or 1,157,826,528 系 MUL flags 构造器 `Or(lo,0)`+`SaveFlags(PF)` carrier,两桶 fail-closed 不借壳重开 lazy flags;RA 能力缺口=0(IR 无 Copy 节点);三桶闭合 delta=0。Apple placement 罚单定因(E1/E2/E3):E1 32B 对齐操纵在地址层面成立(47 个自环点 pad 臂 47/47 对齐,+173 NOP/692B 仅回吐静态删除量 7.72%,NOP 在回边前不进稳态),但全部预注册 A/A 窗口被外部负载风暴作废(loadavg 62、12 个满核 Python shard + WinEmu 190%,最小摆幅 2.52%>2% 门),E2 PMU 无合格双臂样本,E3 同阻——机理诚实未收敛,未以作废窗口报任何 A/B;三开关维持现状(INT_IMM_FOLD ON/KEYGEN_COMPACT OFF/width-chain OFF),下一可证伪实验为 2×2 固定布局×固定寄存器命名(指令条数与热 loop 绝对地址不变,只换 RA 物理家:随名走=rename/WAW,随址走=fetch/cache-set);probe 交付前移除,tracked diff 为空。operand_copy_kill 机制收官并翻默认 ON(`8d53e79` 机制 OFF + `06606f6` 翻盘,=0 回退):EmitMul 对已是 plain 寄存器的 RHS 不再 MaterializeOperand 补拷,A64 mul 三寄存器直供;fail-closed 矩阵=非 Mul/宽 <32-bit/自身 pseudo flags/RHS 带 observer(GetUses(false)==GetUses 且 GetPseudoOperations 为空)/spill/composite/immediate 全退原路。coremark RE=0 动态闭合 1,879,200,194 条(0.724361481% host)digit-exact(指挥官 build-master 独立复放 1,879,200,199,+5 系冷尾 PC 运行间波动,32 个变动 PC 全 ∈{−1,−2} 零负),锚点 0x402df8 42→40/0x402d58 26→25/0x402668 与 0x402814 逐字不变;smallpt/c-ray 闭合 102,788,031/579,940,554,三语料 oracle 两态一致;spill/flags/terminal/fault 指纹全 0 mismatch,helper-fault 两态 38/0 日志逐字同;指纹语料 1,786 unit 无合格 Mul 形态(全部带 flags observer 或 immediate RHS),golden 重写后零 diff 即翻盘 golden-free;默认态套件 1,052,900 断言 172 用例全绿。FeatureSet 68 / env 158。master = `06606f6`。zero_store_zr 收官并翻默认 ON(`c9bc0d6` 机制 OFF + `11a3e04` 翻盘,=0 回退):单用(GetUses(false)==1 含 pseudo 全计数,observer 自然拒绝)且非 spill 的 LoadImm(0) 若唯一 use 是同块普通 StoreUniform/StoreMemory value,EmitLoadImm 抑制物化、store 直发 wzr/xzr(U8/U16/U32→wzr,U64→xzr;vector/composite/非零/多用/CallLambda 全走旧路);GetUses 语义已在 instr.cpp:293-305 实证(exclude_pseudo=false 返回含 pseudo 的 num_use)。coremark 干净 RE=0 臂闭合 **4,594,462,164 条 = 1.603293% host**(指挥官 build-master 独立复放,与 codex 4,594,462,167 仅差 3 条冷尾;701 变动 PC delta ∈ {1,2,3} 零负,锚点 0x402808 15→14、0x4024ec 41→39、0x40ec50 25→24 逐位对);smallpt 闭合 2,288,995,519(1.523279%),c-ray 4,535,211,701(0.983701%),三语料 oracle 两态逐字一致。**池核算口径教训**:zero-store 资格依赖块切割——同一 guest 指令在不同 unit 成形下 LoadImm 多用变单用,故审计池(2.31B,旧成形)与正式臂闭合(4.59B,现成形)差 2×,两者各自内部 digit-exact;跨 harness(IR dump 探针 15.5s vs 密度臂 24.6s,单 unit 数 888 vs 655)的数字不可互减,闭合只信同 harness RE=0 双臂回放。spill/flags/terminal/fault 指纹全 0 mismatch,helper-fault 两态 38/0 逐字同;指纹语料 1,786 unit 无合格形态,golden 重写零 diff 即翻盘 golden-free。FeatureSet 69 / env 159。master = `11a3e04`。 卫生双道收官(57c5a14 + 3062426):道 A 删实验/审计钩子 env 159→151、FeatureSet 69→67,+71/−918——XMM_FPR_POOL_CAP 供给探针、SCRATCH_PRECISE_PEAK_AUDIT、FPCR_TAX 三 env 及 fpcr_tax_prof 490 行(含 RuntimeProfileInterface 计数/timing 缓冲与 EmitHostCall/trampolines 接线)、SIGNAL_PRIVATE_FRAME 已知竞态旧路径(ucontext+fpstate 无条件化)、XMM_SNAPSHOT_DSE gate(正确路径无条件化)、SSE_NAN_FAST approximate 模式;translator_mem.cpp 两处经编译期空 RecordFpcrTaxCounter 兼容标记过渡、translator_alu.cpp 三处 static constexpr false 残枝待解封后机械删除。道 B 首批:interpreter guest flags 四成员函数整搬 interpreter_flags.cpp,位布局常量与三纯 helper 经私有 interpreter_internal.h 共享(匿名命名空间内链),interpreter.cpp 3058→2788 行,纯移动逐字自证。两道默认态发码零变化:指纹两 profile 对既有 golden byte-identical(golden 未重生),全套件 1053188+1053204/173 绿、helper-fault 38/0、九 oracle 全绿、五 gate 失败严格同集(xsave_test 467/468/843/1045 四项既有红灯)。master = `3062426`。 卫生跟进两批(264f00a + 63cc492):道 A 阶段 C 删尽合并过渡残枝——jit_context.h 空方法、fpcr_mode.h 兼容枚举/IgnoredCounter 重载、translator_mem.cpp 两调用点直调六参、translator_alu.cpp 三处 sse_nan_fast 恒假整支,FpcrTaxCounter/sse_nan_fast arm64 后端零引用(+5/−47);道 B B2 EmitMemOperand 整搬 translator_operand.cpp,translator.cpp 2638→2390 行,函数体 SHA-256 逐字自证纯移动,BiasMem 多路径共用留原处未扩边界。两批默认态零变化:套件 1053161+1053203/173 绿、指纹 golden byte-identical 未重生、helper-fault 38/0、五 gate 严格同集(xsave 467/468/843/1045)。master = `63cc492`。 道 B B3(beb9cd3):interpreter ordinary memory 十三成员 + atomic 五 helper 七成员分两批整搬 interpreter_memory.cpp,interpreter.cpp 2788→2380 行;三段搬迁块 SHA-256 我独立复算逐字一致,IsVector 入私有 seam。套件 1053259/173 绿、指纹 golden byte-identical 未重生、五 gate 严格同集。master = `beb9cd3`。 道 B B4(6bc913d):EmitTerminal+局部 condition 七成员整搬 translator_terminal.cpp,删除行与新 TU 多重集逐行一致;region/backedge 共用 helper 留原 TU。指纹 golden byte-identical、五 gate 严格同集;direct-link 并发压力测试 vcpu2 风暴(loadavg 21)误红,w68 同码二进制同风暴同红证伪代码因果,load 7 窗口复跑 900011 assertions 全绿。master = `6bc913d`。 道 B B5(d6920b3):interpreter vector 三家族七小批纯移动——crypto 13 成员→interpreter_vector_crypto.cpp、integer 46 成员→interpreter_vector_integer.cpp、float 22 成员→interpreter_vector_float.cpp,interpreter.cpp 2380→807 行;97 函数 SHA 自证+删除行多重集我独立复算零流失,Linux +crypto 编译属性随迁正确 TU。套件 1053208/173 绿、指纹 golden byte-identical、五 gate 严格同集。master = `d6920b3`。 道 B B6(1f89ef6):region/backedge 25 成员五批整搬 translator_region.cpp(PlanBackedgeFlags 单独成批),translator.cpp 2132→1108 行;25 函数 SHA 自证+多重集我独立复算零流失,flags 家族留 translator_flags.cpp 未跨边界。套件 1053231/173 绿、指纹 golden byte-identical、五 gate 严格同集、region focused 绿。master = `1f89ef6`。 道 B B7(432ad2c):Translate(Block*) 五段提取,主函数 390→69 行;五段文本 SHA 全等,observer 全序 45+68 项前后零 diff;三个出参经查证纯局部量无外部读者。套件 1053221/173 绿、指纹 golden byte-identical、五 gate 严格同集。master = `432ad2c`。 道 B B8(42edd9b):RA 同 TU seam——RunVerified 家族 239 行 .inc 化(段 SHA 我独立复算一致),VReg 八方法 out-of-line 化+私有声明头,Map/Mark/rollback 64 项调用序零 diff,公开头不动。套件 1053286/173 绿(loadavg 111 下仍全绿)、指纹 byte-identical、五 gate 严格同集。master = `42edd9b`。 道 B B9 收官(945bc98):LinearScan 四大函数薄壳化(CacheConstantAddresses 256→31/CoalesceWidthChains 370→30/CoalesceGuestRegisterAccesses 408→30/CoalesceGuestFPRAccesses 239→18),十批逐段整搬 GPR/FPR/const-address 三 TU,register_alloc_pass.cpp→2400 行;段 SHA 十批全等、Map/Mark/rollback 全序十批零 diff、rollback 块未跨条件边界,callback seam 零虚函数;公开头不动。套件 1053206/173 绿、指纹 golden byte-identical、五 gate 严格同集(loadavg 136 下 direct-link 压力测试已知负载误红,同二进制默认态同测绿)。道 B 九批全收官:interpreter.cpp 3058→807、translator.cpp 2638→1108、register_alloc_pass.cpp 约 3.7k→2400,新增 11 TU。master = `945bc98`。
...
**寻址模式发射逐 PC 审计收官（2026-08-04，报告 `/private/tmp/addressing-audit-report.md`，audit-first 无生产改动）**：模式真值（审计时点，当日晚些时候 `3a02f32` 已把 Linux 默认翻为 identity）——两平台默认都是 32-bit bounded bias（`host = pt + (guestEA mod 2^32)`，隔离设计：野指针触不到宿主），`SVM_MEM_IDENTITY=1` 是 Linux-only opt-in 恒等映射，macOS 无此开关。差距三层：①非 bias 路径单条融合早已齐全（`[base,index,LSL#n]`/`[base,imm]`/post-index）；②**bias 下 `pt+base+index` 三输入超 ARM64 两寄存器寻址槽，两条是结构下限**（HOSTBASE_FOLD 后实际形态 = wrapping W add + `[pt,W,uxtw]`）；③标量 composite EA 在 `GetOperand` 提前物化导致 emitter 拿不回 base/index 结构。地址路径多余指令动态账：coremark 3.21% / smallpt 7.03% / STREAM 6.59%（观测上界）。修复点分账：**A** simple GetOperand RA tie（coremark **1.148%**/smallpt 0.254%，当前模式干净可修，与 move 桶重叠）；**C** bias 大立即数 guard 误用 LS SIMM9 范围（smallpt 0.097% peephole）；**D** Linux identity（STREAM 池全部）；**G** smallpt 绝对常量物化 3.93%（非寻址题，需 ADRP/literal/reloc 独立设计）；atomic/TSO bare-address 限制双方共有。**orb 实测 `SVM_MEM_IDENTITY=1`：STREAM 四稳态循环每条访存精确消一条（Copy 42→32/Scale 28→26/Add 30→27/Triad 32→29 host 指令），oracle×3 全绿（CRC 0x25b5/Validates/SHA）**；identity 还释放 x24(pt)+x10(mem_scratch) 两个保留寄存器（与 pin3 scratch 池问题直接相关）。FEX 取证：其 frontend/IR 一直保留 Base/Index/Scale 结构、无 pt bias（Addressing.cpp/MemoryOps.cpp），恒等是它的天然工作点。**修复点 A 已落地并转正（`2820ec8`/`4400aeb`）：A1 实测 coremark −1.06%（审计池 1.148% 内）、smallpt −0.19%（池 0.254% 内）；A2 identity 复合结构保留追加 coremark −1.76%/smallpt −0.60%（identity 合计 −2.83%/−0.79%），spill 零回涨。修复点 G 已落地（`11d8ce7`，`SVM_ABS_CONST_MAT` 当时默认 OFF（现由 `b692fca` 翻为默认 ON））：绝对常量第三条实为 scratch→result transport（非位宽所需），直发 result 后 smallpt 动态 −1.62%(bias)/−1.35%(identity)、静态 −959 条零增长 PC、spill 零回涨；吃掉旧 G 分子 35.4%，残余 ~2.53% 需 ADRP/literal 基建（两路线均已实测否决：code cache 超 ±4GiB 无 publish-time relocation / FinalizeCode 排池扩出 allocation + serializer 拒 literal，重做需 allocation 与 code/data 元数据重构）。残余：C（0.097% peephole）。**
...
- **GPR 全 pin（level 3）× identity 复测（2026-08-04，orb 实测不翻盘，轴维持关闭）**：动机是 identity 把 x24(pt)+x10(mem_scratch) 还回 RA 池（`trampolines.cpp:75-80` 仅 bias 下 Mark；orb `[svm-reg-mask]` 实测 allocatable_gprs：L2i=14、L3i=10、L3b=8）。coremark 150k 三轮中位数：**L2×identity 12,723.7 / L3×identity 9,084.9 / L3×bias 10,222.9 Iterations/Sec**（9/9 CRC `0x25b5` 绿）。level 3 在 identity 下 −28.60% vs level 2，比 bias 下的 −19.66% 更差——+2 寄存器没救回 scratch 枯竭，identity 的融合寻址把 index 寄存器压进池内活跃区间，在 10 寄存器池上反噬。level 2 + XPOOL 维持生产形态。附带发现既有 bug：level 3 + `SVM_EXEC_PROF=1` 在两种内存模式下均 PANIC（"spilling unit reached emission without conditional x18 reservation"），与 identity 无关，level 3 为 opt-in 故仅记录。**2026-08-04 复证该 bug 非 level 3 特有：level 2 默认形态下 smallpt × bias × `SVM_EXEC_PROF=1` 同样命中（新旧两版 master 二进制均复现，identity 模式因多两寄存器不触发 spilling unit 而不崩）；即 EXEC_PROF 抬高 scratch 需求把个别 unit 推进 spilling 路径后缺 x18 条件保留。生产路径（EXEC_PROF 默认 OFF）不受影响，仅记录待修。**
...
| SVM_MEM_IDENTITY | 0/OFF/off/1 | Linux 1（mac 无效） | Linux 默认恒等映射（guest=host 原地址寻址）；=0/OFF/off 显式回退 bounded bias（隔离现场开关）。macOS 不读取、永远 bias。撞图 WARNING 后回退 32-bit 窗口续跑 |
| SVM_MEM_MODE_TRACE | 0/1 | 0 | 加载完成后打印实际 identity/use_memory_base/mask/window/bias 诊断 |
| SVM_EXEC_TRACE | 0/1 | 0 | 故障轨迹探针：每 Runtime 32 项 guest RIP/RSP ring，unhandled fault 时转储（`8386d00`）；默认零开销零行为变化 |
| SVM_ADDR_EA_TIE | 0/1 | 1 | 寻址修复点 A（`2820ec8` 落地、`4400aeb` 转正）：A1 GetOperand 终端别名 RA tie 消 mov；A2 identity 专用 composite EA 结构直进 memory IR（bias 逐字节不变）。=0 现场回退（精确复现旧 IR 形态）。orb 实测 identity A1+A2 coremark −2.83%/smallpt −0.79% 动态 host 字节 |
| SVM_ABS_CONST_MAT | 0/1 | 1 | 修复点 G（`11d8ce7`，`b692fca` 翻默认）：绝对地址常量直接向 GetOperand 结果物化，删 scratch→result transport。当前 orb identity smallpt 动态 −1.87%，spill 零回涨；=0 回退旧代码形状 |
...
**默认态变更（2026-08-04，`3a02f32`）**：Linux 默认翻转为 identity 恒等映射
（恢复原地址寻址设计意图；pt/窗口只是开关的本意就是只为特殊地址空间/软页表
开启）。**Linux 默认态下本节的 bounded 隔离属性不再成立**——guest 野指针可达
宿主既有映射；需要隔离的部署必须显式 `SVM_MEM_IDENTITY=0`（与
`SVM_GUEST_BITS` 显式设置同码路径）。撞图（首个 ET_EXEC span
`MAP_FIXED_NOREPLACE` 失败）不再 fatal：WARNING 后回退 32-bit bounded 窗口
续跑（all-or-nothing 预留，无部分映射混合态）。macOS 不读取 selector，永远
bounded bias（4GB pagezero 硬性阻挡低地址 ET_EXEC 恒等），隔离属性不变。
诊断：`SVM_MEM_MODE_TRACE=1` 打印加载后实际模式。隔离套件已改为显式 =0
运行，21/21 保持。
